...
namespace Mall.Web.Framework
{
    class WeixinOAuth : IMobileOAuth
    {
        public MobileOAuthUserInfo GetUserInfo(ActionExecutingContext filterContext, out string redirectUrl)
        {
            ...
            {
                string code = filterContext.HttpContext.Request.Cookies["code"];
                    ...
                    if (result.Contains("access_token"))
                    {
                        ...
                        if (userStr.Contains("nickname"))
                        {
                            ...
                            userInfo = new MobileOAuthUserInfo()
                            {
                                ...
                                LoginProvider = "Mall.Plugin.OAuth.WeiXin",
                ...
                else //还没有到用户授权页面
                {
                    string url = string.Format("https://open.weixin.qq.com/connect/oauth2/authorize?appid={0}&redirect_uri={1}&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect"
                        , settings.WeixinAppId, System.Web.HttpUtility.UrlEncode(filterContext.HttpContext.Request.Path.ToString()));
        ...
        public MobileOAuthUserInfo GetUserInfo(ActionExecutingContext filterContext, out string redirectUrl, Entities.WXshopInfo settings)
        {
            ...
            if (!string.IsNullOrEmpty(settings.AppId))
            {
                ...
                {
                    string url = string.Format("https://open.weixin.qq.com/connect/oauth2/authorize?appid={0}&redirect_uri={1}&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect"
                        , settings.AppId, System.Web.HttpUtility.UrlEncode(filterContext.HttpContext.Request.Path.ToString()));
                    redirectUrl = url;//指定跳转授权页面 
                }
            }
            return userInfo;
        ...
        /// <returns></returns>
        public MobileOAuthUserInfo GetUserInfo_bequiet(ActionExecutingContext filterContext, out string redirectUrl, Entities.WXshopInfo settings)
        {
            MobileOAuthUserInfo userInfo = null;
            redirectUrl = string.Empty;
