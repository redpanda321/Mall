...
namespace Mall.Web.Framework
{
    public class ApiHelper
    {
        ...
        public static void CheckBaseParamsAndSign(SortedDictionary<string, string> data)
        {
            if (data == null)
            {
                throw new MallApiException(ApiErrorCode.System_Error, "no params");
            }
            if (data.Count < 1)
            {
                throw new MallApiException(ApiErrorCode.System_Error, "no params");
            ...
            {
                throw new MallApiException(ApiErrorCode.Missing_Timestamp, "timestamp");
            }
            ...
            {
                throw new MallApiException(ApiErrorCode.Invalid_Timestamp, "timestamp");
            }
            if (!data.TryGetValue("sign", out sign))
            {
                throw new MallApiException(ApiErrorCode.Missing_Signature, "sign");
            }
            ...
            {
                throw new MallApiException(ApiErrorCode.Invalid_Signature, "sign");
            }
        }
        ...
        public static WxAppletUserInfo GetAppletUserInfo(HttpContext context)
        {
         //   HttpContext context = (HttpContext)curRequest.Properties["MS_HttpContext"];//获取传统context
            string encryptedData =string.Empty;
            string iv = string.Empty;
            string sessionKey = string.Empty;
            if (context.Request.Cookies["encryptedData"] != null && context.Request.Cookies["iv"] != null && context.Request.Cookies["session_key"] != null)
            {
                encryptedData = context.Request.Cookies["encryptedData"].ToString();
                iv = context.Request.Cookies["iv"].ToString();
                sessionKey = context.Request.Cookies["session_key"].ToString();
            }
