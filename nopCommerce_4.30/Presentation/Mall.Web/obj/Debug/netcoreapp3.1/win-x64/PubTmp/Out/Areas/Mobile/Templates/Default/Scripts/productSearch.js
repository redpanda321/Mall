...
function initFilter() {
    $.ajax({
        ...
        success: function (result) {
            if (result.success) {
                //console.log(data);
                renderCategory(result.data.Category);
                renderBrand(result.data.Brand);
                renderAttr(result.data.Attr);
                init();
                initListShowType();
            }
        }
...
function initPrice() {
    $.ajax({
        ...
        success: function (result) {
            if (result.success) {
                limitPrice = result.data.LimitProducts;
                fightPrice = result.data.FightProducts;
                discount = result.data.Discount;
                selfShopId = result.data.SelfShopId;
                FlushPrice();
            }
        }
    });
...
function FlushPrice() {
    ...
    $("#searchList li[shopid='" + selfShopId + "']").each(function () {
        ...
        $(this).find(".red").text("￥" + MoneyRound(parseFloat(showdisprice)));
    });
    //限时购
    $(limitPrice).each(function () {
        $("#searchList li[productid='" + this.ProductId + "']").find(".red").text("￥" + MoneyRound(this.MinPrice));
    });
    //拼团
    $(fightPrice).each(function () {
        $("#searchList li[productid='" + this.ProductId + "']").find(".red").text("￥" + MoneyRound(this.ActivePrice));
    });
}
...
function init() {
    ...
    $('#searchList').on('click', 'li', function () {
        location.href = '/' + areaName + '/product/detail/' + $(this).attr('productId') + '?returnUrl=' + encodeURIComponent(location.href);
    });
...
function loadProducts() {
    ...
    var isSaleCountOnOff = $("#isSaleCountOnOff").val();
    if (!isnodata) {
        page++;
        ...
        $.post('/' + areaName + '/search?' + location.href.split('?')[1],
            ...
            , function (result) {
                if (result.data.length > 0) {
                    var html = '';
                    $.each(result.data, function (i, product) {//图片后台处理
                        ...
                        html+='<a class="p-img" href="/' + areaName + '/product/detail/' + product.ProductId + '?returnUrl=' + encodeURIComponent(location.href)+'"><img src="' + product.ImagePath + '" alt=""></a>\
						<h3><a href="/' + areaName + '/product/detail/' + product.ProductId + '?returnUrl=' + encodeURIComponent(location.href)+'">' + product.ProductName + '</a></h3>\
						<p class="red">¥' + product.SalePrice + '</p>\
						<p>' + product.Comments + '人评价</p>';
                        if (isSaleCountOnOff) {
                            html += '<p>销量：' + Number(product.SaleCount + Number(product.VirtualSaleCounts)) + '</p>';
                        }
                        html += '</li>';
                    });
                    var list = $('#searchList');
