...
namespace Mall.Web.Areas.Admin.Controllers
{
    ...
    public class WeiBigWheelController : Controller
    {
        ...
        {
            query.Type = WeiActivityType.Roulette;
        ...
        {
            var result = WeiActivityApplication.GetActivityWin(text,id, page, rows);
        ...
        public ActionResult Add(FormCollection fc)
        {
            ...
            model.activityType = WeiActivityType.Roulette;
            ...
            if (!string.IsNullOrWhiteSpace(fc["harward"]))
            {
                ...
                for (int i = 1; i <= Convert.ToInt32(fc["harward"]); i++)
                {
                    ...
                    if (awardModel.awardType == WeiActivityAwardType.Integral)
                    {
                    ...
                    if(awardModel.awardType == WeiActivityAwardType.Bonus)
                    {
                    ...
                    if(awardModel.awardType == WeiActivityAwardType.Coupon)
                    {
        ...
        {
            return WeiActivityApplication.GetBonusSurplus(id);
        }
        ...
        private List<SelectListItem> BindDrop(bool Edit)
        {
            ...
            var bonus = WeiActivityApplication.GetBonusByType(Entities.BonusInfo.BonusType.Prize);
            if (Edit)//是否是修改
            {
