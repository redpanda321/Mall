        ...
        [[
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    html += '<a class="good-check" target="_blank" href="/Admin/VTemplate/EditTemplate?client=' + row.client+'&tName=' + row.id + '">编辑</a><a class="good-check" onclick="deleteTopic(' + row.id + ',\'' + row.name + '\')">删除</a>';
        ...
        $.post('./delete', { id: id }, function (result) {
            ...
            if (result.success) {
                $.dialog.succeedTips('删除成功', function () {
                    ...
                    var pageNo = $("#topicGrid").MallDatagrid('options').pageNumber;
                    $("#topicGrid").MallDatagrid('reload', { pageNumber: pageNo });
                });
            }
            else
