    ...
    $("#list").MallDatagrid({
        ...
        [[
            { field: "Id", hidden: true },
            {
                ...
                formatter: function (value, row, index) {
                    var spc = " ";
                    if (row.Color != null && row.Color.length > 0) { spc += "，" + row.ColorAlias + "：" + row.Color; }   //颜色
                    if (row.Size != null && row.Size.length > 0) { spc += "，" + row.SizeAlias + "：" + row.Size; }       //尺寸
                    if (row.Version != null && row.Version.length > 0) { spc += "，" + row.VersionAlias + "：" + row.Version; }   //版本
                    if (spc != " ") { spc = "【" + spc.substr(2) + "】"; }
            ...
            },
        {
            ...
            formatter: function (value, row, index) {
                ...
                if (!row.IsHidden) {
                    html.push("<a onclick=\"deleteComment('" + id + "');\">前台隐藏</a>");
                }
                else {
                    html.push("<a>评价已清除</a>");
                }
