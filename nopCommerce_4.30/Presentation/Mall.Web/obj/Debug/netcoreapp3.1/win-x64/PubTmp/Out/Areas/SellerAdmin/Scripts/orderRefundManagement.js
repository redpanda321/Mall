...
$(function () {
    ...
    datacols = [[
            {
                ...
                formatter: function (value, row, index) {
                    if (row.IsVirtual) {
                        return '<a href="/SellerAdmin/order/Detail/' + value + '" target="_blank">' + value + '<span>(虚拟)</span></a>';
                    } else {
    ...
    datacols[0] = datacols[0].concat([
            ...
            formatter: function (value, row, index) {
                if (row.IsVirtual) {
                    if (row.RefundPayStatus != 1 && row.RefundBatchNo != "") {
                        return "<span class='ftx-04'>退款中</span>";
                    } else {
                        return value;
                    }
                } else {
        ...
        {
            field: "operation", operation: true,width: 90, title: "操作",
            formatter: function (value, row, index) {
    ...
    $('#searchButton').click(function (e) {
        ...
        $("#list").MallDatagrid('clearReload', { startDate: startDate, endDate: endDate, orderId: orderId, productName: productName, userName: userName, shopBranchId: shopBranchId, auditStatus: status });
    })
    ...
    $('.nav-tabs-custom li').click(function (e) {
        ...
        if ($(this).attr('type') == 'statusTab') {//状态分类
            ...
            status = $(this).attr('value') || null;
            $("#list").MallDatagrid('clearReload', { auditStatus: status });
        }
    });
});
    ...
    $.post('./DealRefund', { refundId: refundId, auditStatus: auditStatus, sellerRemark: sellerRemark }, function (result) {
        ...
        if (result.success) {
            ...
            $.dialog.errorTips(result.msg, function () {
                if (result.msg = "需要先设置好退货地址再进行操作，立即前往设置！") {
                    window.location.href = "/Selleradmin/ShopShipper/Management";
                }
            });
        }
    });
...
function ExportExecl() {
    ...
    var href = "/SellerAdmin/OrderRefund/ExportToExcel?startDate={0}&endDate={1}&orderId={2}&productName={3}&userName={4}&shopBranchId={5}&auditStatus={6}&ShowRefundType={7}"
        .format(startDate, endDate, orderId, productName, userName, shopBranchId, status, showtype);
