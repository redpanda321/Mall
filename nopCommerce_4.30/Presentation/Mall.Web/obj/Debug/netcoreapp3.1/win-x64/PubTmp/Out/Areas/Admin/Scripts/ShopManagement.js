...
function Query() {
    ...
    $("#shopDatagrid").MallDatagrid({
        ...
        pageSize: 15,
        pageNumber: 1,
        queryParams: { shopName: shopName, shopAccount: shopAccount, shopGradeId: shopGradeId, status: shopStatus, type: type },
        toolbar: "#shopToolBar",
        columns:
        [[
            ...
            { field: "IsSelf", title: "IsSelf", hidden: true },
            {
                field: "Name", title: "店铺名称", width: 140, formatter: function (value, row, index) {
                    if (row.Status != '被拒绝' && row.Status != '待付款')
                        return '<a target="_blank" href="/Shop/Home/' + row.Id + '">' + row.Name + '</a>';
            ...
            {
                field: "ShopBranchCount", title: "门店数", width: 140, formatter: function (value, row, index) {
                    return '<a href="/Admin/ShopBranch/Management?shopId=' + row.Id + '">' + value + '</a>';
                }
            },
            { field: "EndDate", title: "有效期", width: 140, sort: true },
            {
