...
function checkBranchSkuBuyNum(dom, max) {
    var dom = $(dom);
    var num = 0;
    var result = true;
    try {
...
function editToCartByBranch(sku, num, bid, successfunc, errorfunc) {
    if (!bid || bid <= 0) return;
    $.post('/' + areaName + '/cart/EditBranchProductToCart', {
        skuId: sku,
        count: num,
        shopbranchId: bid
    }, function (result) {
        if (result.success == true) {
            ...
            if (successfunc) {
                successfunc();
                if (GetBranchCartProducts) GetBranchCartProducts(bid);
            }
        } else {
            $.dialog.errorTips(result.msg);
            if (errorfunc) {
        ...
        function (payments) {
            ...
            $.each(paymentType, function (i, payment) {
                ...
                if (!hasAdv && payment.id == 9) {
                    payHtml += '<li class="disabled" >预存款支付（余额不足）</li>';
                } else {
            ...
            html += '</div>';
            html += '<ul class="custom-dialog-body" id="paymentlist">' + payHtml + '</ul>';
            ...
            $("#paymentlist li:not(.disabled)").eq(0).trigger("click");
            $('.custom-dialog-footer .btn').click(function () {
                if (jumpUrl != '') {
                    if (type == 9) {
                        ...
                        //lly 调整
                        loading = showLoading('');
                        $.post(jumpUrl, function (result) {
                            loading.close();
                            if (result.success) {
                                //通过A链接模拟跳转
                            	//jumpUrl在weixin中为一段js脚本，直接调用jsapi
                            	if ($('#payJumpUrl').length > 0) {
                            	    $('#payJumpUrl').attr('href', result.data.jumpUrl);
                            	}
                            	else {
                            	    $(pdlg).append('<a id="payJumpUrl" style="display:none;" href="' + result.data.jumpUrl + '"></a>');
                            	}
