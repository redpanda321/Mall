$.productSelector = {
    params: { auditStatus: 2, saleStatus: 1, selectedProductIds: [], isShopCategory: false, showSku: true, exceptProductIds: [],productType:0 },
    serviceType: 'admin',
    multiSelect: true,
    selectedProducts: [],
    ...
    html: '<div id="_productSelector" class="goods-choose clearfix">\
            <div class="choose-left">\
                <div class="choose-search">\
                	...
                	<div class="form-group">\
                        <input class="form-control input-ssm" type="text" id="" placeholder="商品名称">\
                    </div>\
...
',
    ...
    reload: function (selectedProductIds, exceptProductIds) {
        ...
        if (this.controller == 'coupon') {
            $.productSelector.params.productType = -1;
        }
        var columns = [
        ...
        $("#_productSelector .choose-search button").unbind('click').click(function () {
            ...
            $("#_productSelector table").MallDatagrid('reload', $.productSelector.params);
        });

    },
    select: function (productId, sender) {
        var product = this.loadedProducts[productId];
        if (this.controller == 'collocation') {
            if (product.isOpenLadder) {
                $.dialog.errorTips('阶梯价商品不能参加组合购！');
                return;
            }
            if (!product.isLimit) {
                $.dialog.errorTips('正在进行的限时抢购商品不能参加组合购！');
                return;
            }
        }
    ...
    clear: function () {
        ...
        this.params = { auditStatus: 2, saleStatus: 1, selectedProductIds: [], showSku: true ,productType:0};
    },
    ...
    show: function (selectedProductIds, onSelectFinishedCallBack, serviceType, multiSelect, exceptProductIds, controller) {
        ...
        $('#_productSelector select').MallLinkage({
            url: curl,
            ...
            onChange: function (level, value) {
                ...
                if (level > 0 && !value) {
                    ...
                    categoryId = $('#_productSelector select').MallLinkage('value', parentLevel);
