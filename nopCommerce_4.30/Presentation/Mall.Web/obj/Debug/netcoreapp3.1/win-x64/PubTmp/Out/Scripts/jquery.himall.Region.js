...
(function ($) {
    $.fn.RegionSelector = function (options) {
        ...
        var funs = {
            ...
            IsFinal: function (obj) {
                return obj.data("isfinal");
            },
            Reset: function (obj) {
                var select = obj.find("select:first");
                select.nextAll("select").html('<option value="RegionSelector">请选择</option>');//清空下级所有选择框数据
                select.val("RegionSelector");
                var options = $.data(obj[0], "RegionSelector");
                my.removeData("value");
                $(options.valueHidden).val("").removeAttr("text").removeAttr("level").removeAttr("isFinal");
            }
        }
        ...
        $.data(this[0], "RegionSelector", params);
        //生成选择框
        my.empty();
        ...
        function loadOption(level, parent) {
            ...
            select.html('<option value="RegionSelector">请选择</option>');//清空原有数据
            select.nextAll("select").html('<option value="RegionSelector">请选择</option>');//清空下级所有选择框数据
            ...
            if (value > 0) {//初始化选定值
                $.post(params.urlTree, { id: value }, function (data) {
                    for (var i = maxLevel; i > 0; i--) {
                        ...
                        select.html('<option value="RegionSelector">请选择</option>');
