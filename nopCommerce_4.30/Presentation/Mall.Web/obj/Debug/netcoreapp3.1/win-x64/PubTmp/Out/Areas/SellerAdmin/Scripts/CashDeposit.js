...
$(function () {
        ...
        $.post("PaymentList", { balance: parseFloat($("#balance").val()) }, function (result) {
            ...
            html += '<li style="margin-bottom:20px;">\
                        ...
                        <label style="margin-top:9px;">\
                            <span>帐号余额</span>(<span style="color:red;">￥' + balance + '</span>)\
                        </label>\
                    </li>';
                    ...
                    callback: function () {
                        if ($('input[name="requestUrl"]:checked').length != 0) {
                            ...
                            if ($('input[name="requestUrl"]:checked').attr('urlType') == "1") {
                                ...
                                var toUrl = '/Pay/QRPay/?id=Mall.Plugin.Payment.WeiXinPay_Native&url=' + url;
                                window.open(toUrl);
                            }
                            if ($('input[name="requestUrl"]:checked').attr('urlType') == "2") {
                                ...
                                AccountBalancePay(parseFloat($("#balance").val()));
                            }
                            else {
                                PayMessage();
    ...
    function AccountBalancePay(balance) {
        $.post('AccountBalancePay', { balance: balance}, function (result) {
            if (result.success) {
