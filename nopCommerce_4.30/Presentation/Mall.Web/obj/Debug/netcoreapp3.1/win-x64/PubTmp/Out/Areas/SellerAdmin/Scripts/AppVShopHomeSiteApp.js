...
$('#addBtn').click(function () {
    $.post('/SellerAdmin/App/GetAllHomeProductIds', { platformType: 1 }, function (data) {
        ...
        $.productSelector.show(data, function (selectedProducts) {
            ...
            $.post('/SellerAdmin/App/AddHomeProducts', { productIds: ids.toString(), platformType: 1 }, function (data) {
                loading.close();
                if (data.success)
        ...
        }, 'selleradmin', null, null, null, 'appvshophome');
    });
});
...
$('#searchBtn').click(function () {
    ...
    $("#list").MallDatagrid('reload', { brandName: productName, productName: productName, categoryId: categoryId });
});
$('#btnSerachCoupon').click(function () {
    $("#tableCouponList").MallDatagrid({
        url: 'GetCouponList',
        nowrap: false,
        rownumbers: true,
...
$('#list').on('blur', 'input[name="sequence"]', function () {
    ...
    if (isNaN(sequence)) {
        ...
        $.post('/SellerAdmin/App/UpdateSequence', { id: id, sequence: sequence }, function (data) {
            if (data.success) {
...
function initHomeProduct() {
    $("#list").MallDatagrid({
        url: '/SellerAdmin/App/GetMobileHomeProducts',
        nowrap: false,
        rownumbers: true,
...
function del(id) {
    $.dialog.confirm('确定要从首页删除该商品吗?', function () {
        var loading = showLoading();
        $.post('/SellerAdmin/App/Delete', { id: id }, function (result) {
            loading.close();
            if (result.success) {
                $("#list").MallDatagrid('reload', {});
            }
