...
$('#submit-order').click(function () {
    ...
    if (!recieveAddressId && deliveryType == "2")
        ...
        $('.goods-info[shopid]').each(function () {
            ...
            orderShop.shopBrandId = $("#divshopbranchInfo").attr('shopbranchid');
            if (productType == 1) {
                orderShop.shopBrandId = $("#shopBranchId").val();
            }
...
function submit(model, loading) {
    if (loading == null)
        loading = showLoading();
    var url = '/' + areaName + '/BranchOrder/SubmitOrder';
            ...
            if (result.data.orderIds != null && result.data != undefined) {
                ...
                if (model.isCashOnDelivery && model.orderShops.length == 1) {
                    ...
                    if (result.data.realTotalIsZero) {
                        integralSubmit(result.data.orderIds.toString());
                    }
                    else {
...
function CalcPrice() {
    ...
    if ($("#divDeliveFee").length > 0) {
        ...
        if (delivefeeOpen) {//减配送费
            sum -= parseFloat($("#divDeliveFee").attr("data-fee"));
        }
    }
...
function CalInvoiceRate(orderTotalIntegral) {
    ...
    $('.taxprice').each(function () {
        ...
        if (_rate > 0) {
            ...
            total = total - delivefee;
            var enabledIntegral = $('#userIntegralSwitch').is(':checked');
            if (enabledIntegral) {
                if (orderTotalIntegral > 0) {
                    total = total - orderTotalIntegral;//商品总价（商品价-积分抵扣金额）
                }
            }
