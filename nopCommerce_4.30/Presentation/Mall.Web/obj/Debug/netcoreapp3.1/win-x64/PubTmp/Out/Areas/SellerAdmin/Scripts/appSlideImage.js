$(function () {
    ...
    $("#vshopBannerTable").on("click", '.glyphicon-circle-arrow-up', function () {
        var oriRowNumber = parseInt($(this).attr('rowNumber'));
        var rowIndex = $("#vshopBannerTable").MallDatagrid('getRowByIndex', $(this).parents('tr').prev().attr('Malldatagrid-row-index')).id;
        if (rowIndex > 0) {
            changeBannerSequence(oriRowNumber, rowIndex, function () {
                $("#vshopBannerTable").MallDatagrid('reload', {});
            });
        }
    });
    ...
    $(".table.vshopbanner").on("click", '.glyphicon-circle-arrow-down', function () {
        var oriRowNumber = parseInt($(this).attr('rowNumber'));
        var rowIndex = $("#vshopBannerTable").MallDatagrid('getRowByIndex', $(this).parents('tr').next().attr('Malldatagrid-row-index')).id;
        changeBannerSequence(oriRowNumber, rowIndex, function () {
            $("#vshopBannerTable").MallDatagrid('reload', {});
        });
    });
...
function initSlideImagesTable() {
    ...
    $("#slideImagesTable").MallDatagrid({
        url: '/selleradmin/App/GetSlideImages',
        nowrap: false,
        rownumbers: true,
        NoDataMsg: '没有找到任何轮播图',
        border: false,
...
function DeleteSlideImage(slideImageId) {
    $.dialog.confirm('确定要删除吗？', function () {
        var loading = showLoading();
        $.post('/selleradmin/App/DeleteSlideImage', { id: slideImageId }, function (result) {
            loading.close();
            if (result.success) {
            ...
            $.dialog({
                ...
                init: function () {
                    $("#imgUrl").MallUpload({
                        ...
                        displayImgSrc: result.item.ImageUrl == null ? "" : result.item.ImageUrl,
                        imgFieldName: "imgUrl",
                        dataWidth: 8
                ...
                button: [
                {
                    name: '确认',
                    callback: function () {
                        var photo = $('#imgUrl').MallUpload('getImgSrc');
                        if (photo == "") {
                            $.dialog.tips("请上传轮播图");
                            return false;
                        }
...
function InitSelectDown() {
    $(".j_selectDown li").click(function () {
        var type = $(this).attr("type");
        var current = $(this).parent().parent().parent(),
        	allUrl=window.location.href,
        	domainUrl=window.location.host,
        	resUrl=allUrl.split(domainUrl)[0]+domainUrl;
        if (type == 1) {
            var selectids = [];
            $.productSelector.params.isShopCategory = true;
            $.productSelector.params.productType = -1;//查询所有类型商品，包括实物、虚拟
            $.productSelector.show(selectids, function (selectedProducts) {
                ...
                current.find("input[type=text]").attr("value", resUrl+'/product/detail/' + ids.join(","));
            ...
            $.limitTimeBuySelector.show(selectids, function (selectedProducts) {
                ...
                current.find("input[type=text]").attr("value", resUrl+'/limittimebuy/detail/' + ids.join(","));
                current.find("input[type=text]").attr("disabled", true);
            }, null, false);
        } else {
...
function changeSequence(oriRowNumber, newRowNumber, callback) {
    ...
    $.ajax({
        type: 'post',
        url: '/selleradmin/App/SlideImageChangeSequence',
        cache: false,
        async: true,
...
function initVShopBannerTable() {
    //商品表格
    $("#vshopBannerTable").MallDatagrid({
        url: '/selleradmin/App/GetVShopBanners',
        nowrap: false,
        rownumbers: true,
