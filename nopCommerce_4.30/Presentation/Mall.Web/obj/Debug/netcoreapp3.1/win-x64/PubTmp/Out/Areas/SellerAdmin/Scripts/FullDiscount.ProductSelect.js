...
$.FDProductSelector = {
    ...
    reload: function (selectedIds, activeId) {
        ...
        $("#_FDProductSelector table").MallDatagrid({
            url: url,
            nowrap: false,
            rownumbers: true,
        ...
        $("#_FDProductSelector .choose-search button").unbind('click').click(function () {
            ...
            if ($("#_FDProductSelector input[name=scode]").length > 0) {
                $.FDProductSelector.params.code = $("#_FDProductSelector input[name=scode]").val();
            }
            $("#_FDProductSelector table").MallDatagrid('reload', $.FDProductSelector.params);
        });
    },
    getData: function () {
        return $("#_FDProductSelector table").MallDatagrid('getRows');
    },
    ...
    selectALL: function () {
        var total = $("#_FDProductSelector table").MallDatagrid("data").total;
    ...
    getSelected: function () {
        ...
        if (!$.FDProductSelector.singleSelect) {
            var selecteds = $("#_FDProductSelector table").MallDatagrid('getSelections');
            ...
            products.push($("#_FDProductSelector table").MallDatagrid('getSelected'));
        }
        return products;
    },
    show: function (selectedProductIds, conflict, activeId, onSelectFinishedCallBack) {
        ...
        $.dialog({
            ...
            ok: function () {
                ...
                if (products == null || products.length == 0) {
                    products = $("#_FDProductSelector table").MallDatagrid('getSelections');
                }
        ...
        var curl = "/selleradmin/category/getCategory";
        $('#_FDProductSelector select').MallLinkage({
            url: curl,
            ...
            onChange: function (level, value) {
                ...
                if (level > 0 && !value) {
                    ...
                    categoryId = $('#_FDProductSelector select').MallLinkage('value', parentLevel);
                    if (parentLevel > 0 && !categoryId) {
                        parentLevel--;
                        categoryId = $('#_FDProductSelector select').MallLinkage('value', parentLevel);
                    }
                }
                $.FDProductSelector.params.categoryId = categoryId;
