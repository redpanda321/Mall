...
function Loading(id, title, pic, action, url) {
    $.dialog({
        ...
        ok: function () {
            ...
            var url = $("#url").val();
            var pic = $("#slidePic").MallUpload('getImgSrc');
...
$(function () {
                ...
                },
                {
                    ...
                    formatter: function (value, row, index) {
                        if (row.URL.length > 50) {
                            return row.URL.toString().substr(0, 50) + " ... ";
                        }
                        else {
                            return row.URL;
                        }
    ...
    $(function () {
        ...
        $(".table").on("click", '.glyphicon-circle-arrow-up', function () {
            var that = this;
            var id = $("#slideDatagrid").MallDatagrid('getRowByIndex', $(this).parents('tr').attr('Malldatagrid-row-index')).Id;
            var loading = showLoading();
            ajaxRequest({
                type: 'POST',
