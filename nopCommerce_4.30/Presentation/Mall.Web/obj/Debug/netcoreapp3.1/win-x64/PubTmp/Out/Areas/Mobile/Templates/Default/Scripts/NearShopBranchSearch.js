...
$(function () {
    ...
    window.mySwipe = Swipe(elem, {
        ...
        auto: 3000,
        callback: function (m) {
            $(elem).find('.swipe-dots').children('span').eq(m).addClass('cur').siblings().removeClass('cur')
        },
    });
    ...
    $("#store_list").on("click", ".products a", function () {
        location = shopBranchIndexUrl + "/Index/" + $(this).attr("branchid") + "?keywords=" + encodeURIComponent(keyWords);
...
function loadStoresData() {
    if (lodeEnd)
        return;
    var queryData = {
        pageNo: curpageindex, pageSize: curpagesize, shopId: shopId, fromLatLng: fromLatLng, keyWords: keyWords, url: "Search"
    }
            ...
            if (result.success == false) {
                ...
                if (data) {
                    ...
                    if (data.Models && data.Models.length > 0) {
                        ...
                        if (curpageindex == 1) {
                            lodeEnd = true;
                            $(".divMorenone").html("<img src='/Areas/Mobile/Templates/Default/Images/no-search.png'/><h5>没有搜索结果</h5><h6>换个关键词试试吧</h6>");
                            $(".divMorenone").show();
...
function LoadProductAndSaleCount(branchIds) {
    var queryData = {
        ids: branchIds.join(','), keyWords: keyWords, url: "SearchShopBranchProductAndSaleCount"
    }
    $.ajax({
        type: "GET",
        url: queryData.url,
                ...
                if (result.data) {
                    ...
                    $.each(result.data, function (i, model) {
                        ...
                        $.each(model.products, function (i, p) {
                            if (i < 4) {
                                arr.push('<li> <a href="' + productUrl + model.branchId + '?productId=' + p.Id + '&keywords=' + keyWords + '"><img src="' + p.ImagePath + '/1_100.png" /><span>¥ ' + p.MinSalePrice + '</span></a></li>');
                            } else {
                                index++;
                            }
                        });
...
function bindbranchNameClick() {
    $('.branchName').click(function () {
        ...
        //if (keyWords) {
        //    location.href = './Index/' + $(this).attr('data') + "?keywords=" + keyWords;
        //} else {
        //    location.href = './Index/' + $(this).attr('data');
        //}
    });
