...
(function($) {
	$.fn.MallDistrict = function(options) {
		var defaults = {
		...
		this.each(function() {
			...
			var _this = $(this);
			var district=_this.siblings('.Mall-district');
			
			if (!(params.id.length > 0)) {
				return false;
			}
			var thisId = 'MallDistrict-' + params.id;
			...
			var MallDistrict = '<div class="Mall-district" id="'+thisId+'"><div class="district-hd"><span>请选择省</span></div>'+provinceStr+'<ul class="district-ul city-ul cl"></ul><ul class="district-ul county-ul cl"></ul><ul class="district-ul street-ul cl"></ul></div>';
			...
			if(params.renderTo.find("#"+thisId).length>0){
				...
				params.renderTo.append(MallDistrict);
				_this.addClass('active');
			}
			...
			params.renderTo.on('click','.province-ul a',function() {
				...
				if(parent.length>0){
					...
					$(this).parents('.Mall-district').hide();
					params.closeFn();
				}
			});
