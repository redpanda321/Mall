    ...
    console.log(result)
        if (result.success)
        {
            $(result.data).each(function (idx, el) {
                ...
                var EditorContent = UE.getEditor('contentDesc' + (idx + 1)).ready(function () {
                    //this.setContent($el.content);
                    if ($el.content != null && $el.content.length > 0) {
                        var strcontent = $el.content.replace(/ data-src=\"/g, " src=\"");//微信里素材管理它图片地址改变了，这里显示时改处理；
...
function SaveMaterial()
{
    ...
    $('.item',$('.WX-edit')).each(function (idx, el) {
        ...
        if ($content.length>0)
        {
            ...
            material.push('show_cover_pic:1');
