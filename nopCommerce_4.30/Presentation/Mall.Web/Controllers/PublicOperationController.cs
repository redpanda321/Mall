...
namespace Mall.Web.Controllers
{
    public class PublicOperationController : Controller
    {
        ...
        public ActionResult UploadPic()
        {
            ...
            if (Request.Form.Files.Count == 0) return Content("NoFile", "text/html");
            else
            {
                for (var i = 0; i < Request.Form.Files.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        //Core.MallIO.CreateFile(fname, file.InputStream, FileCreateType.Create);
                        //file.SaveAs(Path.Combine(path, filename));
                        */
                        Core.MallIO.CreateFile(fname, file.OpenReadStream(), FileCreateType.Create);
                    }
                    catch (Exception ex)
                    {
                        Log.Error("上传文件错误", ex);
        ...
        public ActionResult UploadPicToVisual(int tid)
        {
            ...
            else
            {
                for (var i = 0; i < Request.Form.Files.Count; i++)
                {
                    ...
                    string DirUrl = IOHelper.GetMapPath("~/Storage/Plat/PageSettings/Template/" + tid + "/");
                    if (!System.IO.Directory.Exists(DirUrl))      //检测文件夹是否存在，不存在则创建
                    {
                        System.IO.Directory.CreateDirectory(DirUrl);
                    }
        ...
        public ActionResult UploadPicToWeiXin()
        {
            ...
            else
            {
                for (var i = 0; i < Request.Form.Files.Count; i++)
                {
                    ...
                    string DirUrl = IOHelper.GetMapPath("~/temp/");
                    if (!System.IO.Directory.Exists(DirUrl))      //检测文件夹是否存在，不存在则创建
                    {
                        System.IO.Directory.CreateDirectory(DirUrl);
                    }
                    ...
                    try
                    {
                        ...
                        FileStream stream = new FileStream(Path.Combine(DirUrl, filename), FileMode.OpenOrCreate);
        ...
        */
        ...
        {
            string result = "无";
            if (!Mall.Core.Cache.Exists("tt"))
            {
                result = "失效";
                Log.Info("缓存已经失效");
                Mall.Core.Cache.Insert("tt", "zhangsan", 7000);
            }
