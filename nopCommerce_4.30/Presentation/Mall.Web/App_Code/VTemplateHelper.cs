...
namespace Mall.Web
{
    ...
    public static class VTemplateHelper
    {
        ...
        public static string GetTemplate(string client, VTemplateClientTypes type, long shopId = 0)
        {
            ...
            StreamReader sr = new StreamReader(IOHelper.GetMapPath(dataurl), System.Text.Encoding.UTF8);
            try
            {
        ...
        public static MetaInfo GetFileMetaInfo(string fileName)
        {
            ...
            string str = _hostingEnvironment.ContentRootPath + fileName;
        ...
        public static void DownloadTemplate(string client, VTemplateClientTypes type, long shopId = 0)
        {
            ...
            {
                if (!Directory.Exists(IOHelper.GetMapPath(templatePath)))
                {
                    var emptyTemplate = GetTemplatePath("0", type);
                    CopyFolder(IOHelper.GetMapPath(emptyTemplate), IOHelper.GetMapPath(templatePath));
                }
            }
            ...
            if (Core.MallIO.IsNeedRefreshFile(datapath, out metaRemoteInfo))
            {
                var metaLocalFile = GetFileMetaInfo(datapath);
                if (CheckMetaInfo(metaRemoteInfo, metaLocalFile))
                {
                    var dataFileBytes = Core.MallIO.DownloadTemplateFile(datapath);
                    if (null != dataFileBytes)
                    {
                        var strDataContent = Encoding.UTF8.GetString(dataFileBytes);
                        string abDataPath = IOHelper.GetMapPath(datapath);
            ...
            if (Core.MallIO.IsNeedRefreshFile(cshtmlpath, out metaRemoteInfo))
            {
                var metaLocalFile = GetFileMetaInfo(cshtmlpath);
                if (CheckMetaInfo(metaRemoteInfo, metaLocalFile))
                {
                    var cshtmlFileBytes = Core.MallIO.DownloadTemplateFile(cshtmlpath);
                    if (null != cshtmlFileBytes)
                    {
                        var strCshtmlContent = Encoding.UTF8.GetString(cshtmlFileBytes);
                        string abCshtmlPath = IOHelper.GetMapPath(cshtmlpath);
