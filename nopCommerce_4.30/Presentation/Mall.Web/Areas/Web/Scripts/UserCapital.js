...
function typeChoose(val) {
        ...
        dataColumn.push({
            ...
            formatter: function (value, row, index) {
                ...
                if (parseFloat(value) > 0)
                    html = value;
                else if (parseFloat(value) == 0 && (row['SourceType'] == 4 || row['SourceType'] == 5 || row['SourceType'] == 6))  //退款 纯积分支付时 我的账户 账户明细 收入显示0
                    html = value;
                return html;
            }
        });
        ...
        dataColumn.push({
            ...
            formatter: function (value, row, index) {
                if (row['SourceType'] == 4) {
                    ...
                    var html = '退款,订单号:<a target="_blank" href="/OrderRefund/Detail/' + row.SourceData + '">' + row.SourceData + '</a>';
                    if (row["IsExitRefund"] == 0)
                        html = '退款，取消订单:<a target="_blank" href="/UserOrder/Detail/' + row.SourceData + '">' + row.SourceData + '</a>';
                    return html;
                } else if (row['SourceType'] == 6) {
                    var html = '分销佣金,单号:' + row.SourceData;
                    return html;
                }
                else {
                    return value;
    ...
    switch (val) {
        ...
        case 2:
            ...
            dataColumn.push({
                field: "PresentAmount", title: '赠送金额', width: 100, align: 'center',
            });
            ...
            dataColumn.push({
                field: "operate", title: '操作', width: 80, align: 'left', formatter: function (value, row, index) {
                    var html = [];
        ...
        case 3:
            ...
            dataColumn.push({ field: "Id", title: '提现单号', width: 120,align:'left' });
            break;
        case 4:
            ...
            dataColumn.push({
                field: "Id", title: '单号', width: 120,align:'left',
                formatter: function (value, row, index) {
        ...
        case 5:
            ...
            dataColumn.push({
                field: "Remark", title: "备注", width: 200, align: "left",
                formatter: function (value, row, index) {
                    var html = '退款,单号:<a target="_blank" href="/OrderRefund/Detail/' + row.SourceData + '">' + row.SourceData + '</a>';
                    if (row["IsExitRefund"] == 0)
                        html = '退款，取消订单:<a target="_blank" href="/UserOrder/Detail/' + row.SourceData + '">' + row.SourceData + '</a>';
                    return html;
                }
