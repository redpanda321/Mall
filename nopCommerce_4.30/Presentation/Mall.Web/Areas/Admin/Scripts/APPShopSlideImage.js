function DeleteSlideImage(slideImageId) {
    $.dialog.confirm('确定要删除吗？', function () {
        ...
        $.post('/admin/APPShop/DeleteSlideImage', { id: slideImageId }, function (result) {
            loading.close();
            if (result.success) {
...
$(function () {
    ...
    $('#topicSearchButton').unbind('click').click(function () {
        ...
        $("#topicGrid").MallDatagrid('reload', { tagsKeyword: tagsKeyword, titleKeyword: titleKeyword });
    });
});
...
function saveChooseTopicToSlideImg(id) {
    ...
    if (!selectedId)
        ...
        if (id == 0) {
            $('#url').val("http://" + window.location.host + '/m-wap/topic/detail/' + selectedId);
        }
        else {
            $('#gotoUrl').val("http://" + window.location.host + '/m-wap/topic/detail/' + selectedId);
        }
    }
}
...
function submitSlideImage() {
    ...
    try {
        ...
        if (!object.imageUrl)
            ...
            $.ajax({
                type: "post",
                url: '/admin/APPShop/AddSlideImage',
...
function submitImageAd(id)
{
    ...
    $.ajax({
        type: "post",
        url: '/admin/APPShop/UpdateImageAd',
...
function getSlideImage(id)
{
    $.ajax({
        type: "GET",
        url: '/admin/APPShop/GetImageAd/' + id,
        dataType: "json",
        async: false,
        success: function (result) {
            if (result.success) {
