    ...
    ajaxRequest({
        ...
        success: function (data) {
            ...
            if (data.success == true) {
                $.dialog.tips('更新分类的' + actionDesc + '成功.', function () { location.reload(); });
            }
            else {
...
function categoryTextEventBind() {
    ...
    $('.container').on('blur', '.text-name,.text-order,.text-commis', function () {
        ...
        if (id == undefined)
            id = $(this).parents('tr').find('.hidden_id').val();
...
$(function () {
            ...
            success: function (data) {
                ...
                if (data.Successfly === true) {
                    ...
                    for (var i = 0; i < data.Category.length; i++) {
                        ...
                        sub.push('<input class="text-name" type="text" categoryId="' + data.Category[i].Id + '"  value="' + data.Category[i].Name + '"/>');
                        sub.push('</td>');

                        sub.push('<td class="clear-m2"><input class="text-order" categoryId="' + data.Category[i].Id + '" type="text" value="' + data.Category[i].DisplaySequence + '"/></td>');

                        if (data.Category[i].Depth === 3) {
                            sub.push('<td>');
                            sub.push('<input class="text-commis" type="text" categoryId="' + data.Category[i].Id + '" value="' + data.Category[i].CommisRate + '"/>');
                            sub.push('<span >%</span>');
                            sub.push('</td>');
                        }
                        sub.push('<td ></td>');
                        ...
                        sub.push('<a href="./Edit?Id=' + data.Category[i].Id + '" class="edit">编辑</a><a class="delete-classify" categoryId="' + data.Category[i].Id + '">删除</a></span>');
                        sub.push('</td>');
                        sub.push('</tr>');
                        p.after(sub.join(''));
                    }
    ...
    $('.container').on('click', 'td.td-operate .delete-classify', function () {
        ...
        //var id = $(this).parents('td.td-operate').prev('td').prev('td').prev('td').find('.hidden_id').val();
        var id = $(this).attr("categoryId");
    ...
    $(".j_isshow").click(function () {
        var id = $(this).attr("categoryid");
        var isShow = $(this).attr("isshow") == 1 ? true : false;
        updateOrderOrName("UpdateCategoryShow", { id: id, isShow: isShow }, '显示');
    });
    $('input.VirtualProductCheck').change(function () {
        var _this = $(this),
            supportVirtualProduct = _this[0].checked ? true : false,
            categoryId = $(this).attr('categoryId'),
            loading = showLoading();
        $.post('./SetSupportVirtualProduct', { id: categoryId, supportVirtualProduct: supportVirtualProduct }, function (result) {
            loading.close();
            if (!result.success) {
