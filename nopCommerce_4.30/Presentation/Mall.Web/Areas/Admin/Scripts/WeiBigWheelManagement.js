...
function loadGrid() {
    $("#list").MallDatagrid({
        ...
        columns:
        [[
            { field: "activityTitle", title: "大转盘名称" },
            ...
            ,
            {
                field: "operation", operation: true, title: "操作", formatter: function (value, row, index) {
                    var html = "";
                    html += '<span class="btn-a"><a href="/Admin/WeiBigWheel/WinManagement?pid=' + row.Id + '">中奖详情</a></span>';

                    html += '<span class="btn-a"><a href="/Admin/WeiBigWheel/Edit/' + row.Id + '">编辑</a></span>';
                
                    html += '<span class="btn-a"><a href="/Admin/WeiBigWheel/Detail/' + row.Id + '?show=no">链接</a></span>';
