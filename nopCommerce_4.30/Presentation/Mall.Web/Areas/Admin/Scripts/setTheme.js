...
$("#btn_save").on("click", function () {
    ...
    $.post("updateTheme", { templateId: $("#templateId").val(), id: $("#themeid").val(), typeId: typeId, MainColor: $("#MainColor").val(), SecondaryColor: $("#SecondaryColor").val(), WritingColor: $("#WritingColor").val(), FrameColor: $("#FrameColor").val(), ClassifiedsColor: $("#ClassifiedsColor").val() }, function (data) {
        if (data.status > 0) {
            $.dialog.tips("保存成功！");
        } else {
            $.dialog.tips("保存失败！");
