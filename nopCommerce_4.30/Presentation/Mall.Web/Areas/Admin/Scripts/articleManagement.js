...
function batchDelete() {
    ...
    var selected = $("#list").MallDatagrid('getSelections');
    var ids = [];
...
function initArticleSelectors() {
    $('#firstLevel,#secondLevel').MallLinkage({
    ...
    $("#list").MallDatagrid({
        ...
        [[
            ...
            //},
           {
               ...
               formatter: function (value, row, index) {
                   var html = "";
                   html += '<a href="' + row.showurl + '" target="_blank">' + value.replace(/</g, '&lt;').replace(/>/g, '&gt;')+'</a>';
                   return html;
               }},
           { field: "categoryName", title: "分类", width: 85, align: "center" },
           {
               field: "displaySequence", sort: true, title: '排序', width: 90,
               formatter: function (value, row, index) {
                   return ' <input class="text-order" type="text" value="' + value + '" articleId="' + row.id + '" />';
               }
           },
           { field: "addDate", sort: true, title: "添加时间", width: 120, align: "center" },
        {
...
function reload() {
    ...
    $("#list").MallDatagrid('reload', { categoryId: categoryId, titleKeyWords: $.trim($('#title').val()), pageNumber: pageNo });
