...
function init() {
    $("#btnSave").click(function () {
        ...
        $.post("SaveConfig", { addressIds: str, addressIds_city: cityids }, function (result) {
            loading.close();
            if (result.success) {
    ...
    $(".sc1").click(function () {
        ...
        var checked = $(this).prev(".check-all").is(':checked');  
        $(this).parent().siblings().find('input').each(function () {
            this.checked = !checked;
        });
        $(this).prev(".check-all").prop("checked", !checked);
    })

    $(".show-region").click(function () {
    	$('.paymentConfig').find(".hidereginDiv ul").hide();
    ...
    $(".show-region-ckb").change(function () {
        ...
        if (this.checked) {
            parentEl.find("li input[type=checkbox]").prop("checked", true);
        }else {
