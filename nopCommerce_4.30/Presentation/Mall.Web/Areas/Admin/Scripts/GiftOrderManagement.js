        ...
        [[
           {
            ...
            {
                field: "FirstGiftBuyQuantity", title: '数量', width: 50,sort:true},
            {
                field: "OrderDate", title: '下单时间', width: 160,sort:true,
                formatter: function (value, row, index) {
            ...
            {
                field: "TotalIntegral", title: '消费积分', width: 80, sort: true,
                formatter: function (value, row, index) {
                    return value;
                }
...
function reload() {
    var pageNo = showstatus == "" ? $("#list").MallDatagrid('options').pageNumber : 1;
    $("#list").MallDatagrid('reload', { skey: $("#skey").val(), status: showstatus, pageNumber: pageNo });
...
function vieworder(id) {
    ...
    if (ExpressCompanyName != "" & ShipOrderNumber != "") {
        ...
        $.post('/Common/ExpressData/Search', { expressCompanyName: data.ExpressCompanyName, shipOrderNumber: ShipOrderNumber }, function (result) {
            ...
            if (obj.expressName) {
                html += '<tr><td colspan="3"><a href="' + obj.expressUrl + '" target="_blank" id="power" runat="server" style="color:Red;">此物流信息由' + obj.expressName + '提供</a></td></tr>';
            }
            html += "<\/tbody><\/table>";
            expinfo = html;
