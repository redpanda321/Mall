...
function getIsOpenStore() {
    $.get('/' + areaName + '/Branchproduct/GetIsOpenStore', function (result) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success) {
                ...
                if (data.saleStatus == 1 && data.auditStatus == 0) {
                    if (data.stock > 0) {
    ...
    if (len === $(".spec").length) {
        ...
        if (checkBuyNum()) {
            ...
            checkLogin(returnHref, function () {
                //if (canBuy(pid) == false)
                //    return false;
                if (canBuy(pid, num, sku, bid) == false) {
                    return false;
                }
                ...
                var url = '/common/site/pay?area=mobile&platform=' + areaName.replace('m-', '') + '&controller=branchorder&action=submit&skuIds={0}&counts={1}&productType={2}&shopBranchId={3}'.format(sku, num, productType, $("#shopBranchId").val());
                //检测是否开启了手机号强制绑定
