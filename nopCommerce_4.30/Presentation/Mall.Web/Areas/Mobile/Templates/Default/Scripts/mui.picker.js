...
(function ($, window, document, undefined) {
    ...
    Picker.prototype.calcElementItemPostion = function (andGenerateItms) {
        ...
        self.elementItems.forEach(function (item) {
            ...
            if (andGenerateItms) {
                var dataItem = {};
                dataItem.Name = item.innerHTML || '';
                dataItem.Id = item.getAttribute('data-value') || dataItem.Id;
    ...
    Picker.prototype.setItems = function (items, type) {//type用来控制显示的值，other数据格式{Id:1,Name:"测试",Level:1},date用于时间选择器
        var self = this;
        self.items = items || [];
        var buffer = [];
        self.items.forEach(function (item) {
            if (item !== null && item !== undefined && type == "other") {
                buffer.push('<li>' + (item.Name || item) + '</li>');
            } else if (item !== null && item !== undefined && type == "date") {
    ...
    Picker.prototype.getSelectedText = function () {
        var self = this;
        return (self.items[self.getSelectedIndex()] || {}).Name;
    };
    ...
    Picker.prototype.refreshPicker = function (flag) {
        var self = this;
        if(self.options.layer<4){
        	return;
        }
        var width = (100 / self.options.layer);
        if (flag) {
            width = (100 / (self.options.layer + 1));
            self.holder.style.width = width * 2 + '%';
        } else {
            self.holder.style.width = width + '%';
        }
        var temp = self.holder.nextElementSibling;
        while (true) {
            if (!temp) break;
            temp.style.width = width + '%';
            temp = temp.nextElementSibling;
        }
        temp = self.holder.previousElementSibling;
        while (true) {
            if (!temp) break;
            temp.style.width = width + '%';
