...
(function ($) {
    ...
    function request(target, param) {
        var opts = $.data(target, "MallDatagrid").options;
        if (param) {
        ...
        function ajaxRequest() {
            $.ajax({
                ...
                success: function (data) {
                    if (data.models) data.rows = data.models;//新增兼容分页模型
                    if (data.Models) data.rows = data.Models;//兼容大小写
                    if (data.Total) data.total = data.Total;//兼容大小写
                    setTimeout(function () {
                        $(target).MallDatagrid("loaded");
                    }, 0);
    ...
    function getSelectRows(target) {
        var opts = $.data(target, "MallDatagrid").options;
        var data = $.data(target, "MallDatagrid").data;
        var tBody = $(target).find("tbody");
        if (opts.idField) {
            return $.data(target, "MallDatagrid").selectedRows;
        } else {
            var rowAry = [];
            $("tr.MallDatagrid-row-selected", tBody).each(function () {
                var row_index = parseInt($(this).attr("MallDatagrid-row-index"));
    ...
    function selectAll(target) {
        var opts = $.data(target, "MallDatagrid").options;
        var data = $.data(target, "MallDatagrid").data;
        var selectRows = $.data(target, "MallDatagrid").selectedRows;
        var tBody = $(target).find("tbody");
        var rows = data.rows;
        selectRows = [];
        $("div.MallDatagrid-cell-check input[type=checkbox]:enabled", tBody)
            .prop("checked", true).parents('tr').addClass("MallDatagrid-row-selected");
        for (var i = 0; i < rows.length; i++) {
            if (opts.idField) {
                var row = rows[i];
                if ($("div.MallDatagrid-cell-check input[type=checkbox]").eq(i).is(':checked')) {
                    selectRows.push(row);
                }
            }
        }
        $.data(target, "MallDatagrid").selectedRows = selectRows;
    };
    function clearSelectRows(target) {
        ...
        var selectRows = $.data(target, "MallDatagrid").selectedRows;
        var tBody = $(target).find("tbody");
        $("tr", tBody).removeClass("MallDatagrid-row-selected");
        $("div.MallDatagrid-cell-check input[type=checkbox]", tBody).prop("checked", false);
    ...
    function selectRow(target, index) {
        ...
        var tr = $("tr[MallDatagrid-row-index=" + index + "]", tBody);
        if (!tr.hasClass("MallDatagrid-row-selected")) {
            tr.addClass("MallDatagrid-row-selected");
            var ck = $("div.MallDatagrid-cell-check input[type=checkbox]", tr);
    ...
    function unSelectRow(target, index) {
        ...
        var tBody = $(target).find("tbody");
        var tr = $("tr[MallDatagrid-row-index=" + index + "]", tBody);
        var ck = $("tr[MallDatagrid-row-index=" + index + "] div.MallDatagrid-cell-check input[type=checkbox]", tBody);
        tr.removeClass("MallDatagrid-row-selected");
    ...
    function bindCellsEvents(target) {
        var opts = $.data(target, "MallDatagrid").options;
        var total = $.data(target, "MallDatagrid").data.total;
        ...
        $("div.MallDatagrid-cell-check input[type=checkbox]", $(target)).unbind("click").bind("click", function (e) {
            var findIndex = $(this).parent().parent().parent().attr("MallDatagrid-row-index");
            if (opts.singleSelect) {
                clearSelections(target);
    ...
    function loadData(target, _data) {
        ...
        $.data(target, "MallDatagrid").data = _data;
    ...
    function initialPager(target) {
        ...
        if (total > 0) {
            ...
            if (opts.pagination) {
                ...
                htmlArray.push("<a class=\"btn btn-default glyphicon glyphicon-backward beforePageBtn\" href=\"javascript:void(0);\"></a>");
                htmlArray.push("<a class=\"btn btn-default glyphicon glyphicon-forward afterPageBtn\" href=\"javascript:void(0);\"></a>");
                htmlArray.push("<a class=\"btn btn-default btn-ssm jump-to\">指定跳转</a>");
                
                var str='<span class="select-pagesize"> 每页 <select class="btn btn-default btn-ssm mr0 pageSizeSelect">';
                ...
                htmlArray.push(str+'</select> 条</span>');
    ...
    //插件构造函数
    $.fn.MallDatagrid = function (options, param) {
        if (typeof options == "string") {
            return $.fn.MallDatagrid.methods[options](this, param);
        }

        options = options || {};
        return this.each(function () {
            var state = $.data(this, "MallDatagrid");
            var opts;
            if (state) {
                opts = $.extend(state.options, options);
                state.options = opts;
            } else {
                opts = $.extend({}, $.fn.MallDatagrid.defaults, options);
                //console.log(opts);
                $.data(this, "MallDatagrid", {
                    options: opts,
                    selectedRows: [],
                    data: {
                        total: 0,
    ...
    //插件对外公开的所有方法集合
    $.fn.MallDatagrid.methods = {
        options: function (jq) {
            var opts = $.data(jq[0], "MallDatagrid").options;
            return opts;
        },
        data: function (jq) {
            var data = $.data(jq[0], "MallDatagrid").data;
            return data;
        },
        reload: function (jq, param) {
            return jq.each(function () {
                if (!param.pageNumber) param.pageNumber = 1;
                $.data(this, "MallDatagrid").options.pageNumber = param.pageNumber;
                request(this, param);
            });
        },
        ...
        loading: function (jq) {
            return jq.each(function () {
                var opts = $.data(this, "MallDatagrid").options;
        ...
        getRows: function (jq) {
            return $.data(jq[0], "MallDatagrid").data.rows;
        },
    ...
    var renderView = {
        render: function (target, container) {
            var opts = $.data(target, "MallDatagrid").options;
            var rows = $.data(target, "MallDatagrid").data.rows;
            var closFileds = $(target).MallDatagrid("getColumnFields");
            ...
            if (rows == null || rows.length <= 0) {
                ...
                var rowData = {};
                for (var i = 0; i < rows.length; i++) {
                    var cls = (i % 2 && opts.striped) ? "class=\"MallDatagrid-row-alt\"" : "";
                    ...
                    if (opts.childField && rows[i][opts.childField].length > 0) {
                        for (var j = 0; j < rows[i][opts.childField].length ; j++) {
                            ...
                            htmlAry.push("<tr MallDatagrid-row-index=\"" + i + "\" " + cls + " " + strStyle + " ");
        ...
        renderHead: function (target, container) {
            ...
            for (var i = 0; i < closFileds.length; i++) {
                ...
                var content = "",
                	...
                	col = $(target).MallDatagrid("getColumnOption", findFiled),
                	sort = '',
                	sortHtml = '',
        ...
        renderRow: function (target, fileds, rowIndex, rowData) {
            ...
            for (var i = 0; i < fileds.length; i++) {
                ...
                var col = $(target).MallDatagrid("getColumnOption", findFiled);
                if (col) {
                    var className = "";
                    ...
                    if (col.checkbox) {
                        cc.push(" class=\"MallDatagrid-cell-check ");
                    } else {
                        cc.push(" class=\"MallDatagrid-cell ");
                    }
                    cc.push("\">");
                    if (col.checkbox) {
                        if (col.formatter) {
