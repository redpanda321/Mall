...
$(function () {
    ...
    $('#searchBtn').click(function (e) {
        location = "?keywords=" + encodeURIComponent($("#searchInput").val());
        e.preventDefault();
    });
    $("#searchInput").keypress(function (e) {
        if (e.keyCode == 13) {
            location = shopBranchSearchUrl + "?keywords=" + encodeURIComponent($(this).val());
            e.preventDefault();
        }
    });
});
...
function loadStoresData() {
    if (lodeEnd)
        return;
    var queryData = {
        pageNo: curpageindex, pageSize: curpagesize, shopId: shopId, fromLatLng: fromLatLng, tagsId: tagsId, url: tagsSearchUrl
    }
...
function getStoreHtml(item) {
    ...
    arr.push('<span id="branchSaleCount' + obj.Id + '" class="fl"></span>');
...
function LoadProductAndSaleCount(branchIds) {
    var queryData = {
        ids: branchIds.join(','), tagsId: tagsId, url: tagsSearchProductUrl
    }
    $.ajax({
        type: "GET",
        url: queryData.url,
