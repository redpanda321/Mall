...
function CreateMainSkuTable(selectedProducts) {
    ...
    var l = selectedProducts[0].isOpenLadder;
    if (l)
    {
    ...
    var isLimit = selectedProducts[0].isLimit;
    if (!isLimit) {
...
function CreateOtherSkuTable(selectedProducts) {
    ...
    $(selectedProducts).each(function (i, pro) {        
        ...
        if (pro.isOpenLadder) { $.dialog.errorTips('阶梯价商品不能参加组合购！'); return true; }
        ...
        td += "<td>";
        $(pro.skus).each(function (index, item) {
            var skuItem = $('p[data-skuid=' + item.Id + ']');
            var Price = $(skuItem).find("input").eq(1).val() || item.SalePrice;
            var SkuPirce = $(skuItem).find("input").eq(0).val() || item.SalePrice;
            td += "<p class='skutd' data-skuid=" + item.Id + "><input type='text' value=" + SkuPirce + " readonly> <input type='text' value=" + Price + "></p>";
        });
