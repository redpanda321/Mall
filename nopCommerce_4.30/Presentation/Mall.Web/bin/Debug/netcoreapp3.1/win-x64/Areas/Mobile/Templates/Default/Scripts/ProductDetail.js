    ...
    if (len === $("div.spec").length) {
        ...
        if (checkBuyNum()) {
            ...
            checkLogin(returnHref, function () {
                ...
                var url = '/common/site/pay?area=mobile&platform=' + areaName.replace('m-', '') + '&controller=order&action=submit&skuIds={0}&counts={1}&productType={2}'.format(sku, num, productType);
                //检测是否开启了手机号强制绑定
