...
using System.Xml;
using CommonHelper = Mall.Core.Helper.CommonHelper;
...
namespace Mall.Web.Areas.Web.Controllers
{
    ...
    public class InstallerController : Controller
    {
        ...
        public InstallerController(IWebHostEnvironment environment, ISettingService setting,
          INopFileProvider fileProvider  ,  IConfiguration configuration,IWebHelper webHelper)
        {
        ...
        private void SetServicesConfig(string ServiceSuffix)
        {
            try
            {
                ...
                string siteMapPath = _hostingEnvironment.ContentRootPath + Request.Path.ToString();
        ...
        private bool TestPermission()
        {
            ...
            try
            {
                ...
                if (config.ConnectionStrings.ConnectionStrings["mysql"].ConnectionString == "none")
                    config.ConnectionStrings.ConnectionStrings["mysql"].ConnectionString = "required";
                else
                    config.ConnectionStrings.ConnectionStrings["mysql"].ConnectionString = "none";
            ...
            // 检查Storage目录的权限
            /////////////////////////////////////////////////////////////////////////////////////////////////////////
            testPath = _hostingEnvironment.ContentRootPath + "/storage/test.txt";
            if (!TestFolder(testPath, out errorMsg))
            {
                errorMsgs.Add(errorMsg);
            }
        ...
        private string GetConnectionString()
        {
            ...
            return String.Format(
                "server={0};database={1};user id={2};password={3};persistsecurityinfo=True;SslMode=none",
        ...
        private string GetSimpleConnectionString()
        {
            ...
            return String.Format(
                "server={0}{1};user id={2};password={3};persistsecurityinfo=True;database={4};Allow User Variables=True;",
                _dbServer, port == 0 ? "" : "," + _dbPort, _dbLoginName, _dbPwd, _dbName);
        }
        ...
        private bool CreateDtabase(out string msg)
        {
            string connection = String.Format(
                "server={0};user id={1};password={2};AllowPublicKeyRetrieval=True;persistsecurityinfo=True;SslMode=none",
            ...
            using (DbConnection con = new MySqlConnection(connection))
            {
                ...
                com.CommandText = "CREATE DATABASE " + _dbName + " CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci";
        ...
        {
            string filename = IOHelper.GetMapPath("/SqlScripts/Schema.sql");
        ...
        private bool ExecuteScriptFile(string pathToScriptFile, out string errorMsg)
        {
            ...
            string strSql;
            string applicationPath = Request.Path.ToString();
        ...
        {
            string filename = IOHelper.GetMapPath("/SqlScripts/SiteDemo.zh-CN.sql");
            if (!System.IO.File.Exists(filename))
            {
        ...
        /// </summary>
        /// <returns></returns>
        public JsonResult ClearDemoData()
        {
            var siteSetting = Mall.Application.SiteSettingApplication.SiteSettings;
            if (!siteSetting.IsCanClearDemoData)
            {
                return Json(new { success = false, msg = "不能执行此操作！" });
            }
            string filename = IOHelper.GetMapPath("/SqlScripts/ClearDemoData.sql");
            string errorMsg = string.Empty;
            if (!System.IO.File.Exists(filename))
            {
                return Json(new { success = false, msg = "没有找到清除演示数据文件-ClearDemoData.Sql" });
            }
            try
            {
                StreamReader reader = null;
                ...
                //初始服务所在根目录
                Configuration config = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
                var conString = config.ConnectionStrings.ConnectionStrings["mysql"].ConnectionString;
                if (string.IsNullOrWhiteSpace(conString))
                {
                    return Json(new { success = false, msg = "数据库连接配置异常！" });
                }
                ...
                return Json(new { success = true, msg = "操作执行成功！" });
            }
            catch (Exception ex)
            {
                errorMsg = "清除演示数据异常";
        ...
        private bool CreateAdministrator(out string errorMsg)
        {
            ...
            try
            {
                using (connection = new MySqlConnection(GetConnectionString()))
                {
                    ...
                    long shopId = 1;
                    dbCmd.CommandText =
                        "SELECT Id FROM Mall_Shop limit 1";
                    ...
                    dbCmd.CommandText =
                        "INSERT INTO Mall_Manager  (shopId, RoleId, UserName, Password, PasswordSalt, CreateDate)" +
                    ...
                    dbCmd.CommandText = "SET FOREIGN_KEY_CHECKS=0;" +
                       "INSERT INTO Mall_Member  (id,UserName, Password, PasswordSalt,TopRegionId,RegionId,OrderNumber,Disabled,Points,Expenditure,CreateDate,LastLoginDate)" +
                    ...
                    dbCmd.Parameters.Clear();
                    dbCmd.CommandText =
                       "update Mall_SiteSetting set Value=@SiteName WHERE  `Key`='SiteName'";
        ...
        private bool UpdateSliderImage(out string errorMsg)
        {
            ...
            try
            {
                using (connection = new MySqlConnection(GetConnectionString()))
                {
                    ...
                    //DbFactory.Default.Set<ImageAdInfo>().Where(p => p.Id == 1).Set(p => p.ImageUrl, "../Images/464x288.png");
                    //DbFactory.Default.Set<ImageAdInfo>().Where(p => p.Id == 8).Set(p => p.ImageUrl, "../Images/464x288.png");
                    //DbFactory.Default.Set<ImageAdInfo>().Where(p => p.Id == 13).Set(p => p.ImageUrl, "../Images/310x165.png");
                    //DbFactory.Default.Set<ImageAdInfo>().Where(p => p.Id > 1 && p.Id < 8).Set(p => p.ImageUrl, "../Images/226x288.png");
                    //DbFactory.Default.Set<ImageAdInfo>().Where(p => p.Id > 8 && p.Id < 13).Set(p => p.ImageUrl, "../Images/226x288.png");
                    dbCmd.Parameters.Clear();
                    dbCmd.CommandText = "update Mall_ImageAd set ImageUrl=@ImageUrl WHERE  `Id`>1 and `Id`<8";
                    dbCmd.Parameters.Add(new MySqlParameter("@ImageUrl", "../Images/226x288.png"));
                    dbCmd.ExecuteScalar();
                    dbCmd.Parameters.Clear();
                    dbCmd.CommandText =
                       "update Mall_ImageAd set ImageUrl=@ImageUrl WHERE  `Id`=8";
                    dbCmd.Parameters.Add(new MySqlParameter("@ImageUrl", "../Images/464x288.png"));
                    dbCmd.ExecuteScalar();

                    dbCmd.Parameters.Clear();
                    dbCmd.CommandText =
                       "update Mall_ImageAd set ImageUrl=@ImageUrl WHERE  `Id`>8 and `Id`<13";
                    dbCmd.Parameters.Add(new MySqlParameter("@ImageUrl", "../Images/226x288.png"));
                    dbCmd.ExecuteScalar();

                    dbCmd.Parameters.Clear();
                    dbCmd.CommandText =
                       "update Mall_ImageAd set ImageUrl=@ImageUrl WHERE  `Id`=1";
                    dbCmd.Parameters.Add(new MySqlParameter("@ImageUrl", "../Images/464x288.png"));
                    dbCmd.ExecuteScalar();

                    dbCmd.Parameters.Clear();
                    dbCmd.CommandText =
                       "update Mall_ImageAd set ImageUrl=@ImageUrl WHERE  `Id`=13";
                    dbCmd.Parameters.Add(new MySqlParameter("@ImageUrl", "../Images/310x165.png"));
                    dbCmd.ExecuteScalar();
        ...
        {
            string imagepath =IOHelper.GetMapPath("/SqlScripts/Storage");
            string destpath = IOHelper.GetMapPath("/Storage");
            try
            {
            ...
            string filename = IOHelper.GetMapPath("/SqlScripts/SiteInitData.zh-CN.Sql");
        ...
        private bool SaveConfig(out string errorMsg)
        {
          ...
          */
                ...
                CommonHelper.AddOrUpdateAppSetting<bool>("Mall:isIntalled", true);

                CommonHelper.AddOrUpdateAppSetting<string>("Mall:CurDomainUrl", CurrentUrlHelper.CurrentUrl());

                CommonHelper.AddOrUpdateAppSetting<string>("Mall:ConnectionString", GetSimpleConnectionString());
