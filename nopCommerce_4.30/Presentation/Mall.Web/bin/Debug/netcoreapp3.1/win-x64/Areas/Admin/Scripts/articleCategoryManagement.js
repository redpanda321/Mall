        ...
        success: function (data) {
            ...
            if (data) {
                ...
                for (var i = data.length-1; i >=0 ; i--) {
                    ...
                    sub.push('<a onclick="edit(' + data[i].Id + ')" href="javascript:;">编辑</a><a class="delete-classify"  onclick="del(' + data[i].Id + ')">删除</a>');
                    sub.push('<a href="/Admin/Article/Add?cid=' + data[i].Id + '">新增文章</a>');
                    sub.push('</span>');
                    sub.push('</td>');
                    sub.push('</tr>');
                    p.after(sub.join(''));
