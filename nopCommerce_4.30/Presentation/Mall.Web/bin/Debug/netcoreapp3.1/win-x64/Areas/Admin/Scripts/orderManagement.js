...
$(function () {
    ...
    $("#list").MallDatagrid({
        ...
        pagePosition: 'bottom',
        pageNumber: 1,
        queryParams: { status: status, paymentType: 0, shopBranchName: $('#txtShopBranchName').val() || null },
        operationButtons: "#orderOperate",
        rowHeadFormatter: function (target, index, row) {
            return '<tr class="child-title"><td colspan="7" style="padding:10px 15px; background:#fff;border:0;font-size: 12px; color:#6b6c6e;"><img src="' + row.IconSrc + '" title="' + (row.PlatformText == 'Android' ? 'App' : row.PlatformText) + '订单' + '" width="16" style="margin:0 8px 0 0;position:relative;top:-1px;" /> 订单号： ' + row.OrderId + (row.IsVirtual == true ? '&nbsp;<span style="margin-left:10px">(虚拟)</span>' : '') + (row.OrderType == 3 ? '<span style="margin-left:10px">(拼团订单)</span>' : '') + ' &nbsp;&nbsp;&nbsp;&nbsp; ' + row.OrderDate + '<span class="pull-right">' + (row.PaymentTypeStr || '') + '</span></td></tr>';
        },
		...
		{ field: "ShopName", title: "商家/门店", width: 140, align: "center" },
    ...
    $('#searchButton').click(function (e) {
        ...
        var shopBranchName = $.trim($('#txtShopBranchName').val());
        ...
        $("#list").MallDatagrid('reload', {
            startDate: startDate, endDate: endDate, orderId: orderId, shopName: shopName, shopBranchName: shopBranchName,
            orderType: orderType, userName: userName, paymentTypeGateway: paymentTypeGateway, paymentType: paymentType,
            userContact: txtUserContact, IsVirtual: isVirtual, InvoiceType: invoiceType
        });
    })
    ...
    $('.nav-tabs-custom li').click(function (e) {
        ...
        if ($(this).attr('type') == 'statusTab') {//状态分类
            $('#txtOrderId').val('');
            $('#txtShopName').val('');
            $('#txtShopBranchName').val('');
            $('#txtuserName').val('');
            $(".search-box form")[0].reset();
            $("#list").MallDatagrid('clearReload', { status: $(this).attr('value') == 0 ? null : $(this).attr('value'), shopBranchName: $.trim($('#txtShopBranchName').val()) });
        }
    });
});
...
function ExportExecl() {
    ...
    var href = "/Admin/Order/ExportToExcel?status={0}&startDate={1}&endDate={2}&orderId={3}&shopName={4}&userName={5}&paymentTypeGateway={6}&paymentType={7}&userContact={8}&orderType={9}&shopBranchName={10}&IsVirtual={11}&invoiceType={12}"
        .format(status, startDate, endDate, orderId, shopName, userName, paymentTypeGateway, selectPaymentType, txtUserContact, orderType, shopBranchName, isVirtual, invoiceType);
...
function AutoComplete() {
    ...
    $('#txtShopBranchName').autocomplete({
        source: function (query, process) {
            var matchCount = this.options.items;//返回结果集最大数量
            $.post("/Admin/Order/GetShopAndShopBranch", { "keyWords": $('#txtShopBranchName').val(), "type": 2 }, function (respData) {
                return process(respData);
            });
        },
        formatItem: function (item) {
