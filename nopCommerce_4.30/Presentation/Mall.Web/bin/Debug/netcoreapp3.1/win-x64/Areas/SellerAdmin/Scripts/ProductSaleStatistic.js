...
function LoadData(date1, date2)
{
    ...
    $("#list").MallDatagrid({
        ...
        columns: [[
             ...
             {
                 field: "SinglePercentConversionString", sort: true, title: "单品转化率", width: 'auto', align: "center"
             },
...
function ExportExecl() {
    var Sort;var IsAsc;
    var object = $("th.select");
    if (object != null) {
        if (object.hasClass("state_switch")) {
            IsAsc = true;
        } else {
            IsAsc = false;
        }
        Sort = object.attr("data-sort");
    }
    var href = "/SellerAdmin/Statistics/ExportProductStatistic?startDate=" + startDate + "&endDate=" + endDate + "&sort=" + Sort + "&isAsc=" + IsAsc;
