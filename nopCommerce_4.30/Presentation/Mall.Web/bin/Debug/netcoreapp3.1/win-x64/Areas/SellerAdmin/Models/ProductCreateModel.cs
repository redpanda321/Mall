...
namespace Mall.Web.Areas.SellerAdmin.Models
{
    ...
    public class ProductCreateModel : DTO.Product.Product
    {
        ...
        public ProductLadderPrice[] Prices { get; set; }
        ...
        /// SKU是否可编辑
        /// </summary>
        public bool AllowSKUEdit { get; set; }
        ...
        /// 当前参加的活动
        /// </summary>
        public string CurrentJoinActive { get; set; }
        #endregion
        ...
        public SellerSpecificationValue[] GetSellerSpecification(long shopId, long productTypeId)
        {
            ...
            if (this.UpdateSpecs != null && this.UpdateSpecs.Length > 0)
            {
                return this.UpdateSpecs.Select(item => new SellerSpecificationValue()
                {
                    ...
                    Value = string.IsNullOrWhiteSpace(item.Value) ? "" : HttpUtility.HtmlDecode(item.Value)
                }).ToArray();
            }
    ...
    public class ProductCreateValidator : AbstractValidator<ProductCreateModel>
    {
        public ProductCreateValidator()
        {
            ...
            RuleFor(p => p.FreightTemplateId).NotNull().When(NonVirtualProduct).WithMessage("请选择运费模版").GreaterThan(0).WithMessage("请选择运费模版");
            ...
            RuleFor(p => p.GoodsCategory).SetValidator(new GoodsCatgoryValidator("商家分类不能为空"));

        }
        private bool NonVirtualProduct(Product model)
        {
            return model.ProductType == 0;
        }
    }
    ...
    public class GoodsCatgoryValidator : PropertyValidator
    {
        public GoodsCatgoryValidator(string errorMessage)
            : base(errorMessage)
        {
        }
        ...
        protected override bool IsValid(PropertyValidatorContext context)
        {
            var pics = context.PropertyValue as long[];
            return pics != null && pics.Any(p => (p > 0));
        }
    }
