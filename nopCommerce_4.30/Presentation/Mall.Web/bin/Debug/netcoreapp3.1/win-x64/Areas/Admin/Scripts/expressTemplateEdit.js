    ...
    ; (function (win, doc) {
        ...
        doc.getElementById('ids').onclick = function () {
            ...
            jQuery.ajax({
                type: 'POST',
                url: 'Save',
                data: { elements: JSON.stringify(arr), name: expressTemplateName, width: jQuery('#imageweight').val(), height: jQuery('#imageheight').val(), backimage: jQuery('#id_img').val() },
                dataType: "json",
                success: function (data) {
                    if (data.success)
...
{
	$.dialog.confirm('确定要清除此快递单的模板内容吗？', function () {
		$.ajax({
	        type: 'get',
	        url: 'ClearData',
	        data: { id: id },
	        ...
	        success: function (data) {
	            if (data.success) {
	                $.dialog.tips('清除成功!', function () {
	                    window.location.reload();
	                });
	            }
	            else
	                $.dialog.alert('清除失败！' + data.msg);
	        }
	    });
	});
}
...
$(function () {
    $('#backgroundImg').MallUpload({
        title: '单据背景图：',
        imageDescript: '805*480',
        displayImgSrc: $("#id_img").val(),
        imgFieldName: "backgroundImg",
        dataWidth: 6,
        headerWidth: 3,
        callback: function (data) {
        	$('#canvas').css('backgroundImage','url('+data+')');
            $('#id_img').val(data);
        }
    });
});