                ...
                $.post("/" + areaName + "/Member/IsConBindSms", null, function (result) {
                    if (result.success) {
                        $.post('../CheckBuyNumber', { id: activeid, skuId: sku, count: num }, function (result) {
                            if (result.success) {
                                hasbuy = result.data.hasbuy;
                                if (hasbuy + num <= maxnum) {
                                    //var _url = "/" + areaName + "/Order/SubmitGroup?skuId=" + sku + "&count=" + num + "&GroupActionId=" + activeid ;
                                    var _url = '/common/site/pay?area=mobile&platform=' + areaName.replace('m-', '') + '&controller=order&action=SubmitGroup&skuId={0}&count={1}&groupActionId={2}&returnUrl={3}'.format(sku, num, activeid, returnHref);
                                    if (groupid) {
                                        var canjoin = false;
                                        $.ajax({
                                            type: "post",
