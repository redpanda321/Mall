        ...
        [[
           {
            ...
            {
                field: "AddDate", title: '添加时间', width: 100,sort:true,
                formatter: function (value, row, index) {
                    return date_string(value);
                }
            },
            {
                field: "EndDate", title: '兑换截止', width: 100, sort: true,
                formatter: function (value, row, index) {
                    return date_string(value);
                }

            },
            { field: "NeedIntegral", title: '积分', width: 80,sort: true},
            { field: "ShowLimtQuantity", title: '限兑', width: 60,sort: true},
            { field: "StockQuantity", title: '库存', width: 60, sort: true},
            {
                field: "RealSales", title: '销量', width: 50, sort: true,
                formatter: function (value, row, index) {
...
function reload(page) {
    ...
    $("#list").MallDatagrid('reload', { skey: $.trim($('#title').val()), status: $('#status').val(), pageNumber: pageNo });
