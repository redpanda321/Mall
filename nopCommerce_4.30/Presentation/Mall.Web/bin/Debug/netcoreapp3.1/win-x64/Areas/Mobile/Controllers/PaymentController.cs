...
namespace Mall.Web.Areas.Mobile.Controllers
{
...

    {
        ...
        {
            OrderApplication.PayByCapital(UserId, ids, payPwd, Request.Host.ToString());
        ...
        public JsonResult Pay(string pmtid, string ids)
        {
            ...
            if (orders == null || orders.Count == 0) //订单状态不正确
            {
                ...
                return ErrorResult<dynamic>("未找到可支付的订单");
            }
            ...
            {
                var openIds = _iMemberService.GetOpenIdByUser(CurrentUser.Id);
            ...
            {
                throw new MallException("错误的支付方式,获取支付地址为空");
            }
        ...
        {
            var items = _iOrderService.GetOrderItemsByOrderId(orders.Select(p => p.Id));
