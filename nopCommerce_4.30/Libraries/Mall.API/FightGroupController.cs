    ...
    public class FightGroupController : BaseApiController
    {
        ...
        public object GetActiveDetail(long id, long grouId = 0, bool isFirst = true, string ids = "")
        {
            ...
            if (result != null)
            {
                ...
                //商品图片地址修正
                result.ProductDefaultImage = MallIO.GetRomoteProductSizeImage(data.ProductImgPath, 1, (int)ImageSize.Size_350);
                result.ProductImgPath = MallIO.GetRomoteProductSizeImage(data.ProductImgPath, 1);
            }
            ...
            #region 商品规格
            var product = ServiceProvider.Instance<IProductService>.Create.GetProduct((long)result.ProductId);
            ...
            model.ProductImagePath = string.IsNullOrWhiteSpace(data.ProductImgPath) ? "" : MallIO.GetRomoteProductSizeImage(data.ProductImgPath, 1, (int)Mall.CommonModel.ImageSize.Size_350);
         ...
         //   AutoMapper.Mapper.CreateMap<FightGroupActiveModel, FightGroupActiveResult>();
            ...
            if (fightGroupData != null && fightGroupData.ActiveItems.Count() > 0)
            {
                ...
                //if (shopItem.IsSelf)//自营店会员有级别比例,活动不折扣
                //{
                //    min = min * discount;
                //    max = max * discount;
                //}
