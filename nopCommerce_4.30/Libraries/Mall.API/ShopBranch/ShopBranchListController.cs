    ...
    public class ShopBranchListController : BaseApiController
    {
            ...
            )
        {
            ...
            if (query.FromLatLng.Split(',').Length != 2)
                return ErrorResult("无法获取您的当前位置，请确认是否开启定位服务！");
        ...
        /// <returns></returns>
        /// 
        [HttpGet("GetStoreHome")]
        public object GetStoreHome(long shopBranchId)
        {
            var shopBranch = ShopBranchApplication.GetShopBranchById(shopBranchId);
            if (shopBranch == null)
                return ErrorResult("获取当前门店信息错误！");

            shopBranch.ShopImages = string.IsNullOrWhiteSpace(shopBranch.ShopImages) ? "" : Core.MallIO.GetRomoteImagePath(shopBranch.ShopImages);
        ...
        public object GetProductList(int pageSize, int pageNo, string shopCategoryId, string shopId, string shopBranchId)
        {
            ...
            var productlist = pageModel.Models.ToList().Select(item =>
            {
                return new
                {
                    ...
                    SaleCounts = item.SaleCounts + Mall.Core.Helper.TypeHelper.ObjectToInt(item.VirtualSaleCounts),//销量统计没有考虑订单支付完成。
                    RelativePath = Core.MallIO.GetRomoteProductSizeImage(item.RelativePath, 1, (int)Mall.CommonModel.ImageSize.Size_350),//150-350
                };
            });
            var result = new
            {
