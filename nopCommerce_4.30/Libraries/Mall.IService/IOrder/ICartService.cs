...
namespace Mall.IServices
{
    public interface ICartService : IService
    {
        ...
        /// <returns></returns>
        List<ShoppingCartInfo> GetCarts(long memberId, int top);
        int GetCartCount(long memberId);
        ...
        /// <returns></returns>
        List<ShoppingCartItem> GetCartItems(IEnumerable<long> cartItemIds);
        ...
        /// <returns></returns>
        List<ShoppingCartItem> GetCartItems(IEnumerable<string> skuIds, long memberId);

        long GetCartProductQuantity(long memberId, long productId = 0, string skuId = "");
        List<ShoppingCartItem> GetCartQuantityByIds(long memberId, IEnumerable<long> productIds);
    }
}
