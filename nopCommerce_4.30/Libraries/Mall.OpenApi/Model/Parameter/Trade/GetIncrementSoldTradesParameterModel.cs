    ...
    public class GetIncrementSoldTradesParameterModel : BasePageParameterModel
    {
        ...
        public override bool CheckParameter()
        {
            ...
            if (start_modified.HasValue)
            {
                ...
                {
                    throw new MallApiException(OpenApiErrorCode.Time_Start_Now, "start_modified");
                }
                if (end_modified.HasValue)
                {
                    if (start_modified.Value >= end_modified.Value)
                    {
                        throw new MallApiException(OpenApiErrorCode.Time_Start_End, "start_modified");
                    }
                    if ((end_modified.Value-start_modified.Value).TotalHours>24)
                    {
                        throw new MallApiException(OpenApiErrorCode.Time_StartModified_AND_EndModified, "start_modified,end_modified");
                    }
                }
            }
            ...
            {
                //throw new MallOpenApiException(OpenApiErrorCode.Time_End_Now, "end_modified");
            }
