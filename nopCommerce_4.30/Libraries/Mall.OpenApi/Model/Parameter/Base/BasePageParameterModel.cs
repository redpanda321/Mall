    ...
    public class BasePageParameterModel : BaseParameterModel
    {
        ...
        public override bool CheckParameter()
        {
            ...
            if (result)
            {
                if (this.page_no < 1)
                {
                    throw new MallApiException(Hishop.Open.Api.OpenApiErrorCode.Invalid_Arguments, "page_no");
                }
                if (this.page_size < 1)
                {
                    throw new MallApiException(Hishop.Open.Api.OpenApiErrorCode.Invalid_Arguments, "page_size");
                }
                if (this.page_size > 100)
                {
                    throw new MallApiException(Hishop.Open.Api.OpenApiErrorCode.Page_Size_Too_Long, "page_size");
                }
            }
            return result;
        }
