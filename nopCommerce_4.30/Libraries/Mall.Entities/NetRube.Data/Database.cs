...
namespace NetRube.Data
{
    ...
    public class Database : IDisposable
    {
        ...
        public Database(IDbConnection connection)
        {
            ...
            CommonConstruct(_connectionString);
        }
        ...
        public Database(string connectionStringName)
        {
            ...
            {
                throw new InvalidOperationException("在配置文件中没有找到名为“{0}”的数据源连接设置".F(connectionStringName));
            }
            ...
            */
            ...
            if (configuration["Mall:ConnectionString"] != null)
            {
                if (!string.IsNullOrEmpty(configuration["Mall:ProviderName"]))
                    providerName = configuration["Mall:ProviderName"];
            }
            else
            {
                throw new InvalidOperationException("在配置文件中没有找到名为“{0}”的数据源连接设置".F(connectionStringName));
            ...
            // Store factory and connection string
            _connectionString = configuration["Mall:ConnectionString"];
            _providerName = providerName;
            CommonConstruct(_connectionString);
        ...
        public virtual bool OnException(Exception x,string msg="")
        {
            System.Diagnostics.Debug.WriteLine(x.ToString());
            System.Diagnostics.Debug.WriteLine(LastCommand);
            var error = "Depth:" + _sharedConnectionDepth + "\r\n" + LastCommand + "\r\n" + x.Message;
            if (!string.IsNullOrEmpty(msg))
                error = msg +"\r\n"+ error;
            Mall.Core.Log.Error(error, x);
            return true;
        }
        ...
        {
            var time = DateTime.Now - CommandBegin;
            if (time.TotalMilliseconds > 3000)
                Mall.Core.Log.Info(string.Format("超时SQL:{0}毫秒,{1}", time.TotalMilliseconds, LastCommand));
            ...
            if (_httpContextAccessor != null)
            {
                ...
                {
                    var executed = (int)(_httpContextAccessor.HttpContext.Items["MysqlExecuted"] != null ? _httpContextAccessor.HttpContext.Items["MysqlExecuted"] : 0);
                    executed++;
                    _httpContextAccessor.HttpContext.Items["MysqlExecuted"] = executed;
                }
            }
            }
        }
            ...
            try
            {
                ...
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, sql, args))
                    {
                        ...
                        if (val.IsDBNull_()) return default(T);
        ...
        {
            var model = poco is IModel ? poco as IModel : null;
            var ret = Update(tableName, primaryKeyName, poco, primaryKeyValue, null == model ? null : model.ModifiedColumns);
        ...
        {
            var model = poco is IModel ? poco as IModel : null;
            var ret = Update(tableName, primaryKeyName, poco, null == model ? null : model.ModifiedColumns);
            if (null != model) model.ModifiedComplete();
            return ret;
        }
        ...
        /// <param name="poco">要更新的实体</param>
        /// <returns>返回受影响的行数，返回 -1 表示执行命令出错</returns>
        public int Update<T>(T poco) where T : IModel
        {
            var model = poco as IModel;
            if (model.ModifiedColumns.Count() <= 0) return 0;
            var ret = Update(poco, null, null == model ? null : model.ModifiedColumns);
            if (null != model) model.ModifiedComplete();
            return ret;
        }
        ...
        {
            return Update(poco, primaryKeyValue, poco is IModel ? (poco as IModel).ModifiedColumns : null);
        }
        ...
        {
            var pd = PocoData.ForType(typeof(T));
            return Execute(new Sql(string.Format("UPDATE {0}", _dbType.EscapeTableName(pd.TableInfo.TableName))).Append(sql.ToString(), sql.Arguments));
        }
        #endregion
        ...
        /// <param name="poco">要更新的实体</param>
        /// <returns>返回受影响的行数，返回 -1 表示执行命令出错</returns>
        public int Delete(dynamic poco)
        {
            var pd = PocoData.ForType(poco.GetType());
        ...
        {
            var pd = PocoData.ForType(typeof(T));
            return Execute(new Sql(string.Format("DELETE FROM {0}", _dbType.EscapeTableName(pd.TableInfo.TableName))).Append(sql.ToString(), sql.Arguments));
        }
        #endregion
        ...
        public bool IsNew(string primaryKeyName, object poco)
        {
            ...
            else
            {
                ...
                if (pi == null)
                    throw new ArgumentException("在实体 {0} 中没有找到名为 “{1}” 的主键".F(poco.GetType(), primaryKeyName));
        ...
        public bool IsNew(object poco)
        {
            ...
            if (!pd.TableInfo.AutoIncrement)
                throw new InvalidOperationException("在实体 {0} 中定义自增主键".F(pd.TableInfo.TableName));
        ...
        public bool Save(string tableName, string primaryKeyName, dynamic poco)
        {
            if (IsNew(primaryKeyName, poco))
            {
                var obj = Insert(tableName, primaryKeyName, true, poco);
                return null != obj;
            }
            else
            {
                var count = Update(tableName, primaryKeyName, poco);
                return count > 0;
            }
        }
        ...
        public void WriteLog()
        {
            try
            {
                Mall.Core.Log.Debug("时间: {0}\t数据库类型:{1}{2}{3}{2}{2}"
