...
namespace NetRube.Data.Internal
{
    ...
    abstract class DatabaseType
    {
        ...
        /// <returns></returns>
        public virtual string EscapeNewName(string newName)
        {
            return string.Format(" AS {0} ", newName);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public virtual string EscapeSqlConcat()
        {
            return "{0} + {1}";
        }
        ...
        public virtual string BuildTopSql(int take, bool dist, string rownoname, string selectColumns, string tableName, string joins, string where, string groupby, string having, string orderby, List<object> args)
        {
            var sql = "SELECT {0}{1} FROM {2}{3} {4}{5}{6}{7} LIMIT @{8}"
                .F(
            ...
            args.Add(take);
            if (take > 1 && !string.IsNullOrEmpty(rownoname)) return "SELECT @{0}:=@{0}+1 AS {0},rownotable.* FROM ({1}) rownotable,(SELECT @{0}:=0) {0}".F(rownoname, sql);
            return sql;
        }
        ...
        /// <param name="args">SQL 查询用的参数</param>
        /// <returns>最终可以执行的 SQL 查询语句</returns>
        public virtual string BuildPagedSql(long skip, long take, bool dist, string rownoname, string selectColumns, string tableName, string joins, string where, string groupby, string having, string orderby, List<object> args)
        {
            var sql = "SELECT {0}{1} FROM {2}{3} {4}{5}{6}{7} LIMIT @{8} OFFSET @{9}"
                .F(
                    dist ? "DISTINCT " : string.Empty,
                    selectColumns,
                    tableName,
