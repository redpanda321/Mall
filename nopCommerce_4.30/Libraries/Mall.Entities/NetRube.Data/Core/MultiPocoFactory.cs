...
namespace NetRube.Data.Internal
{
	class MultiPocoFactory
	{
					...
					for(int i = 1; i < types.Length; i++)
					{
						...
						for(int j = i - 1; j >= 0; j--)
						{
							...
							if(candidates.Count() > 1)
								throw new InvalidOperationException("无法自动连接实体 {0}".F(types[i], types[j]));
						...
						if(!handled)
							throw new InvalidOperationException("无法自动连接实体 {0}".F(types[i]));
		...
		static Delegate FindSplitPoint(Type typeThis, Type typeNext, string ConnectionString, string sql, IDataReader r, ref int pos)
		{
			...
			throw new InvalidOperationException("无法在查询的结果中分离出 {0} 和 {1} 两个实体".F(typeThis, typeNext));
