...
namespace NetRube.Data.DatabaseTypes
{
    ...
    class OracleDatabaseType : DatabaseType
    {
        ...
        public override string BuildTopSql(int take, bool dist, string rownoname, string selectColumns, string tableName, string joins, string where, string groupby, string having, string orderby, System.Collections.Generic.List<object> args)
        {
            var sql = "SELECT {0} FROM (SELECT {1}{0} FROM {2}{3} {4}{5}{6}{7}) WHERE ROWNUM <= @{8} ORDER BY ROWNUM ASC"
                .F(
                    selectColumns,
        ...
        public override string BuildPagedSql(long skip, long take, bool dist, string rownoname, string selectColumns, string tableName, string joins, string where, string groupby, string having, string orderby, System.Collections.Generic.List<object> args)
        {
            if (selectColumns.StartsWith("*"))
                throw new Exception("执行分页查询时不能直接使用“*”来获取所有字段，必需加上表别名。例如：select t.* from table t order by t.id");

            // Same deal as SQL Server
            return Singleton<SqlServerDatabaseType>.Instance.BuildPagedSql(skip, take, dist, rownoname, selectColumns, tableName, joins, where, groupby, having, orderby, args);
        }
        #endregion
    }
}