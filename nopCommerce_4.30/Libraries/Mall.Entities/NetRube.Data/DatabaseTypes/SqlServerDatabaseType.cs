...
namespace NetRube.Data.DatabaseTypes
{
    ...
    class SqlServerDatabaseType : DatabaseType
    {
        ...
        public override string BuildTopSql(int take, bool dist, string rownoname, string selectColumns, string tableName, string joins, string where, string groupby, string having, string orderby, List<object> args)
        {
            var sql = "SELECT {0}TOP (@{1}) {2} FROM {3}{4} {5}{6}{7}"
                .F(
        ...
        /// <param name="args">SQL 查询用的参数</param>
        /// <returns>最终可以执行的 SQL 查询语句</returns>
        public override string BuildPagedSql(long skip, long take, bool dist, string rownoname, string selectColumns, string tableName, string joins, string where, string groupby, string having, string orderby, List<object> args)
        {
            var sql = "SELECT {0} FROM (SELECT ROW_NUMBER() OVER ({1}) __RowNum, {0} FROM {2}{3} {4}{5}) __Paged WHERE __RowNum > @{6} AND __RowNum <= @{7}"
                .F(
                    selectColumns,
