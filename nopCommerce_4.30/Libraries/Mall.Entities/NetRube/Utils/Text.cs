...
namespace NetRube
{
	public static partial class Utils
	{
		...
		{
			if(str.IsNull()) return EmptyArray<string>();
		...
		public static string GetSubString(this string srcString, int length, string tailString = null)
		{
		...
		{
			return srcString.GetSubString(length * 2, tailString);
		}
		...
		public static string GetMatch(this string str, string pattern, bool ignoreCase = false)
		{
			if(str.IsNullOrEmpty_()) return string.Empty;
		...
		{
			return ToRMB_(num, Constants.CurrencyDigital, Constants.CurrencyUnit, Constants.CurrencyOutOfRange);
		}
		...
		{
			if(ip.IsNullOrEmpty_() || ipList.IsNullOrEmpty()) return false;
