...
namespace Mall.Core
{
    ...
    public class MessageQueue
    {
        ...
        private static void Load()
        {
            try
            {
                imessageQueue =  EngineContext.Current.Resolve<IMessageQueue>();

            }
            catch (Exception ex)
            {
                throw new  MallException("注册队列服务异常", ex);
            }
        ...
        private static string ProcessQueueName(string queueName)
        {
            ...
            {
                queueName = string.Format("{0}_{1}", Mall.Core.Helper.WebHelper.GetIP(), queueName);
            }
            return queueName;
        }
        /// <summary>
