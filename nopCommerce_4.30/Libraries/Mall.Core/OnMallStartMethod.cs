...
namespace Mall.Core
{
    ...
    public class OnMallStartMethodAttribute:Attribute
    {
        /// <summary>
        /// 没有返回值的空参数签名
        /// </summary>
        ...
        public OnMallStartMethodAttribute(Type type, string methodName, int order = 0)
        {
        ...
        static void StartImp()
        {
            ...
            var thisType = typeof(OnMallStartMethodAttribute);
            ...
            var MallStartMethodAttributes = allAttributes.Select(item => item as OnMallStartMethodAttribute).OrderBy(item => item.Order);
            foreach (var atrribute in MallStartMethodAttributes)
            {
