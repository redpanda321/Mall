...
namespace Mall.Service
{
    ...
    public class ShopOpenApiService : ServiceBase, IShopOpenApiService
    {
        ...
        public ShopOpenApiSettingInfo MakeOpenApi(long shopId)
        {
            if (shopId <= 0)
            {
                throw new MallException("[OpenApi]错误的店铺编号");
            }
            ...
            {
                throw new MallException("[OpenApi]店铺已生成AppKey,不可以重复生成");
            }
        ...
        public void Add(ShopOpenApiSettingInfo data)
        {
            ...
            {
                throw new MallException("[OpenApi]店铺不可拥有多个AppKey");
            }
            ...
            {
                throw new MallException("[OpenApi]AppKey已存在");
            }
            if (string.IsNullOrWhiteSpace(data.AppKey))
            {
                throw new MallException("[OpenApi]AppKey不可以为空");
            }
            if (string.IsNullOrWhiteSpace(data.AppSecreat))
            {
                throw new MallException("[OpenApi]AppSecreat不可以为空");
            }
        ...
        public void SetEnableState(string appkey, bool state)
        {
            ...
            if(data==null)
            {
                throw new MallException("[OpenApi]错误的appkey");
            }
            data.IsEnable = state;
            Update(data);
        }
        ...
        public void SetEnableState(long shopId, bool state)
        {
            ...
            if (data == null)
            {
                throw new MallException("[OpenApi]错误的shopId");
            }
            data.IsEnable = state;
            Update(data);
        }
