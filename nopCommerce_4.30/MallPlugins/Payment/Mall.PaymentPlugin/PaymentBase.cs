...
namespace Mall.PaymentPlugin
{
    public class PaymentBase<T> where T : ConfigBase, new()
    {
        ...
        public static IHttpContextAccessor _httpContextAccessor = EngineContext.Current.Resolve<IHttpContextAccessor>();
        ...
        void EnableOrDisable(PlatformType platform, bool enable)
        {
            ...
            if (!config.SupportPlatfoms.Contains(platform))//检查是否支持该类型
                throw new Mall.Core.PlatformNotSupportedException(platform);
        ...
        /// <returns></returns>
        public virtual PaymentInfo ProcessRefundReturn(IHttpContextAccessor context)
        {
            throw new PluginException("未实现此方法");
        }
        ...
        /// <exception cref="ApplicationException"></exception>
        /// <returns></returns>
        public virtual PaymentInfo ProcessRefundNotify(IHttpContextAccessor context)
        {
            throw new PluginException("未实现此方法");
        }
