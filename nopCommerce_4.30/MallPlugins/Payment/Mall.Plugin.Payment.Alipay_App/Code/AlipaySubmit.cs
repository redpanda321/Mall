...
using System.Xml;
using Mall.Plugin.Payment.Alipay.Base;

namespace Mall.Plugin.Payment.Alipay_WAP.Code
{
    /// <summary>
    /// 功能：支付宝各接口请求提交类
    ...
    public class Submit
    {
        ...
        private static string BuildRequestMysign(Dictionary<string, string> sPara,Config config)
        {
            ...
            switch (config.Sign_type)
            {
                case "MD5":
                    ...
                    break;
                case "RSA":
                    mysign = RSAFromPkcs8.sign(prestr, config.Private_key, config.Input_charset);
                    break;
                case "0001":
                    mysign = RSAFromPkcs8.sign(prestr, config.Private_key, config.Input_charset);
                    break;
        ...
        private static Dictionary<string, string> BuildRequestPara(Dictionary<string, string> sParaTemp,Config config)
        {
            //待签名请求参数数组
            ...
            //获得签名结果
            mysign = BuildRequestMysign(sPara,config);
        ...
        private static string BuildRequestParaToString(Dictionary<string, string> sParaTemp, Encoding code,Config config)
        {
            //待签名请求参数数组
            Dictionary<string, string> sPara = new Dictionary<string, string>();
        ...
        public static string BuildRequestUrl(Dictionary<string, string> sParaTemp,Config config)
        {
            string strPara = BuildRequestParaToString(sParaTemp, Encoding.GetEncoding(config.Input_charset), config);
            string strUrl = config.GateWay + "?" + strPara;
            return strUrl;
        }
        /// <summary>
        /// 建立请求，以表单HTML形式构造（默认）
        ...
        public static string BuildRequest(string GATEWAY_NEW, Dictionary<string, string> sParaTemp, string strMethod, string strButtonValue,Config config)
        {
            //待请求参数数组
            ...
            sbHtml.Append("<form id='alipaysubmit' name='alipaysubmit' action='" + GATEWAY_NEW + "_input_charset=" + config.Input_charset + "' method='" + strMethod.ToLower().Trim() + "'>");
        ...
        public static string BuildRequest(string GATEWAY_NEW, Dictionary<string, string> sParaTemp,Config config)
        {
            ...
            //待请求参数数组字符串
            string strRequestData = BuildRequestParaToString(sParaTemp, code, config);
            //把数组转换成流中所需字节数组类型
            byte[] bytesRequestData = code.GetBytes(strRequestData);

            //构造请求地址
            string strUrl = GATEWAY_NEW + "?_input_charset=" + config.Input_charset;
        ...
        /// <returns>支付宝处理结果</returns>
        public static string BuildRequest(string GATEWAY_NEW, Dictionary<string, string> sParaTemp, string strMethod, string fileName, byte[] data, string contentType, int lengthFile,Config config)
        {
        ...
        public static Dictionary<string, string> ParseResponse(string strText,Config config)
        {
            //以“&”字符切割字符串
            ...
            if (dicText.ContainsKey("res_data"))
            {
                ...
                {
                    dicText["res_data"] = RSAFromPkcs8.decryptData(dicText["res_data"], config.Private_key, config.Input_charset);
                }
