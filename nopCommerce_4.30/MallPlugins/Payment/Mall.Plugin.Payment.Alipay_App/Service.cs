...
namespace Mall.Plugin.Payment.Alipay_App
{
    public class Service : _ServiceBase, IPaymentPlugin
    {
        ...
        {
            //Post方式
            IFormCollection coll = context.HttpContext.Request.Form;
            ...
            string notifyid = context.HttpContext.Request.Form["notify_id"];//获取notify_id
            string sign = context.HttpContext.Request.Form["sign"];//获取sign
            var signStr = "";
        ...
        {
            //Get方式
            IQueryCollection coll = context.HttpContext.Request.Query;
            Dictionary<string, string> paras = new Dictionary<string, string>();
            foreach (string key in coll.Keys)
            {
                paras.Add(key, coll[key]);
            ...
            if (isSign)
            {
                info.OrderIds = coll["out_trade_no"].ToString().Split(',').Select(item => long.Parse(item));
