...
namespace Mall.Plugin.Payment.WeiXinPay_Native
{
    /// <summary>
    /// 微信原生支付(Native)，采用模式二
    /// </summary>
    public class WeiXinPay : ServiceBase, IPaymentPlugin
    {
        ...
        public string GetRequestUrl(string returnUrl, string notifyUrl, string orderId, decimal totalFee, string productInfo, string openId = null)
        {
            ...
            paraHandler.SetParameter("spbill_create_ip", _httpContextAccessor.HttpContext.Features.Get<IHttpConnectionFeature>().RemoteIpAddress.ToString());
